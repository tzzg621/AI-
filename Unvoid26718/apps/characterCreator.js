 ...
 */
export async function createCharacterByAI(description, { callAIFn, presetId } = {}) {
    if (!callAIFn) {
        throw new Error('缺少 AI 调用函数');
    }
...
{
    "name": "角色名称",
    "emoji": "一个合适的 emoji 图标",
    "desc": "简短的角色描述，20字以内",
    "secret": "一个内心秘密",
    "style": "说话风格描述",
    "stats": { "力量": 数值1-99, "智力": 数值1-99, "魅力": 数值1-99 }
}`;

    // ★ 改成用新的 callAI 签名
    const result = await callAIFn({
        systemPrompt: systemPrompt,
        assistantContext: userContent,
        presetId: presetId
    });
    // 尝试解析 JSON（AI 可能加 markdown 代码块标记）
    let data;
    try {
        // 先试试直接解析
    ...
    return {
        ...
        secret: data.secret || '',
        style: data.style || '',
        stats: data.stats || { 力量: 50, 智力: 50, 魅力: 50 }
    };
}
