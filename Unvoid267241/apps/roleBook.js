...
function createCardHTML(item, index) {
    ...
    return `
        <div class="role-card" data-index="${index}" 
            ...
            : `<span style="font-size:40px; color:#bbb; opacity:0.5;">${item.base.name.charAt(0)}</span>`
        }
            </div>
            ...
            <div style="flex:1; display:flex; flex-direction:column; align-items:center; justify-content:center; padding:8px 6px; width:100%;">
                ...
                <div style="font-size:11px; color:#999; margin-top:2px; text-align:center;">
                    ${item.base.gender || ''}${item.base.gender && item.base.age ? ' · ' : ''}${item.base.age || ''}
                </div>
                ` : ''}
            </div>
...
function renderDetailView(index) {
    ...
    return `
        <div class="screen-page">
            <div class="screen-header">
                <div class="screen-title">${chara.base.name}</div>
                <div class="header-spacer"></div>
            </div>
            <div class="screen-content">
                <div class="page-card">
<div style="text-align:center; margin:8px 0;">
    ...
    <div style="font-size:14px; color:#6a1b9a; white-space:pre-wrap;">${chara.base.detail || '无'}</div>
</div>
...
function renderEditForm(index) {
    ...
    <textarea id="editDetail" rows="6"
              style="width:100%; border:1px solid #ccc; border-radius:8px; padding:10px; font-size:14px; resize:vertical; box-sizing:border-box; font-family:inherit;">${chara.base.detail || ''}</textarea>
</div>
...
export function bindEvents(container, { memoryService, globalState }) {
    ...
    if (viewingIndex !== null) {
        ...
        deleteBtn?.addEventListener('click', async () => {
            ...
            // ① 删除 CharacterStore 数据（聊天、好友、记忆）
            try {
                localStorage.removeItem('char_' + charId);
            } catch (e) { /* 忽略 */ }

            // ② （不删图片，保留在相册/图库中）

            // ③ 从角色列表中移除
            characters.splice(viewingIndex, 1);
            saveCharacters(characters);

            // ④ 如果删除的是当前主视角，重置 activeIndex
            if (viewingIndex === activeIndex) {
                activeIndex = -1;
                saveActiveIndex(-1);
            } else if (viewingIndex < activeIndex) {
                activeIndex--;
            ...
            showToast(`✅ 已删除「${chara.base.name}」`, '#e53935');
        });
