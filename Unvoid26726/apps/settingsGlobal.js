...
export function bindGlobalEvents(container, onBack) {
    ...
    container.querySelector('#togglePhoneFrameBtn')?.addEventListener('click', () => {
        const current = localStorage.getItem('global_show_phone_frame') !== 'false';
        localStorage.setItem('global_show_phone_frame', current ? 'false' : 'true');

        // 直接切换手机壳的显示
        const wrapper = document.querySelector('.phone-wrapper');
        if (wrapper) {
            wrapper.style.background = current ? 'transparent' : '#1c1c1c';
            wrapper.style.padding = current ? '0' : '12px 8px';
            wrapper.style.borderRadius = current ? '0' : '40px';
            wrapper.style.boxShadow = current ? 'none' : 'var(--shadow)';
            const screen = wrapper.querySelector('.phone-screen');
            if (screen) {
                screen.style.borderRadius = current ? '0' : '30px';
            }
        }
