...
${messages.map((msg) => {
        ...
        // const msgBubbles = msg.text.split('|').map(s => s.trim()).filter(s => s);
        const sentences = (msg.text || '').split('|').map(s => s.trim()).filter(s => s);
        const bubbleHtml = sentences.length > 1
            ? sentences.map(t => `<div class="msg-bubble ${isMe ? 'me' : 'other'}">${t}</div>`).join('')
            : `<div class="msg-bubble ${isMe ? 'me' : 'other'}">${msg.text}</div>`; if (isMe) {
...
`;
        ...
        setTimeout(async () => {
            ...
            const activeDisplayName = globalState?.activeCharacter?.base?.name || '我';  // ★ 加这行
            const { systemPrompt, assistantContext } = buildPrompt({
                ...
                targetId: activeId,            // ★ 主视角ID
                targetName: activeDisplayName,   // ★ 主视角名字
            ...
            if (localStorage.getItem('auto_memory_' + otherId) === 'true') {
                ...
                if (noteMatch) {
                    ...
                    // 写入 AI 扮演的角色（otherId）对主视角（activeId）的认知
        ...
        <div style="min-width:0;">
            ...
            ">
                ...
                <div style="
                    ...
                    padding:0 12px;
                ">
                    <div style="padding:0 0 10px 0;line-height:1.6;">${reasoningContent}</div>
                </div>
            </div>
            <style>
                .reasoning-expanded {
...
`;
            ...
            const msgContentDiv = aiMsg.querySelector('div[style="min-width:0;"]');  // 气泡容器
    ...
    container.querySelector('#extractMemoryBtn')?.addEventListener('click', async () => {
        ...
        try {
            ...
            await extractMemoriesForActiveChar(activeId, otherId, contact.name, allMessages);
