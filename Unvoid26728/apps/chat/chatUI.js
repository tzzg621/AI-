    ...
    import('../../store/CharacterStore.js').then(({ CharacterStore }) => {
        ...
        overlay.innerHTML = `
            <div style="background:white; border-radius:20px; width:300px; padding:20px;">
                <div style="font-weight:600; font-size:16px; margin-bottom:4px;">📝 我对${otherName}的认知</div>
                ...
                <textarea id="cognitiveText" rows="6" placeholder="……"
                          style="width:100%; border:1px solid #ccc; border-radius:8px; padding:8px; font-size:13px; resize:vertical; box-sizing:border-box; font-family:inherit;">${currentNote}</textarea>
...
function showWindowSummary(context) {
    ...
    overlay.innerHTML = `
        <div style="background:white;border-radius:20px;width:320px;padding:20px;">
            ...
            <div style="font-size:12px;color:#888;margin-bottom:12px;">${activeName} 与 ${otherName} 的对话总结</div>
    ...
    extractBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            const store = new CharacterStore(activeId);
            const messages = store.getMessages(pairKey);
