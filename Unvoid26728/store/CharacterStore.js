...
export class CharacterStore {
    ...
    updateRelation(name, relation, perspective, id) {
        ...
        const existing = rels.find(r => r.name === name);
        if (existing) {
            existing.relation = relation;
            if (perspective !== undefined) existing.perspective = perspective;
    ...
    updateAttitudes(name, attitudes) {
        const rels = this.getRelations();
        const existing = rels.find(r => r.name === name);
        if (existing) {
    ...
    removeRelation(name) {
        this.setRelations(this.getRelations().filter(r => r.name !== name));
    }
