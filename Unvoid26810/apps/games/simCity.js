...
async function showCityChat(container, roleId, profile, friendId, placeName, persist = true, fpPre, gcIdPre) {
        ...
        <div style="background:white;border-radius:18px 18px 0 0;width:100%;max-height:70%;display:flex;flex-direction:column;box-shadow:0 -10px 40px rgba(0,0,0,0.2);">
            <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:1px solid #f0f0f0;">
                <div style="display:flex;align-items:center;gap:8px;">
                    ...
                    <div>
                        <div style="font-size:14px;font-weight:700;" id="ccTitle">${esc(friendName)}</div>
    ...
    const groupMode = !persist;                 // 只有临时（地点）对话开群聊
    ...
    function renderHeader() {
        ...
        if (!titleEl) return;
        if (!groupMode) { titleEl.textContent = friendName; return; }
        const names = participants
            .filter(id => id !== roleId)
            .map(id => id === friendId ? friendName : charName(id));
    ...
    function renderMsgs() {
        ...
        msgsEl.innerHTML = messages.map(m => {
            if (m.system || m.from === 'system') {
                return `<div style="text-align:center;font-size:11px;color:#aaa;margin:8px 0;">${esc(m.text)}</div>`;
            }
            const mine = m.from === roleId;
            const name = mine ? myDisplay : (m.from === friendId ? friendName : charName(m.from));
            return `<div style="display:flex;flex-direction:column;${mine ? 'align-items:flex-end;' : 'align-items:flex-start;'}margin-bottom:8px;">
            <div style="font-size:10px;color:#bbb;margin:0 4px 2px;">${esc(name)}</div>
    ...
    async function send(text) {
        ...
        if (m) {
            const place = PLACES.find(p => p.name === m[1] || m[1].startsWith(p.name));
        ...
        try {
            ...
            const nowStatus = statusNow(friendId, hour);

            const history = messages.slice(-20).map(m => {
                if (m.system || m.from === 'system') return `【系统】${m.text}`;
                return `${m.from === roleId ? promptMyName : (groupMode ? charName(m.from) : friendName)}：${m.text}`;
            }).join('\n');
            ...
            // ★ 在场者名单（per-角色授权：只放TA自己的公开信息 + TA自己的游戏好友）
            const rosterLines = [];
            for (const pid of participants) {
                ...
                // 与「我」的关系（relations 授权，和现有 myInfo 同一套）
                const rel = isContact ? await readGameData(roleId, 'relations', () => new CharacterStore(roleId).getRelationById(pid)) : null;
                if (rel?.relation) line += `，和「我」的关系：${rel.relation}`;
                // 该角色自己的游戏好友（contacts 授权，从TA自己的 gameFriends 推）
                const ownFriends = (pf.gameFriends || [])
                    .filter(f => participants.includes(f.id) && f.id !== roleId && f.id !== pid)
                    .map(f => charName(f.id));
                if (ownFriends.length) line += `，游戏好友：${ownFriends.join('、')}`;
            ...
            const reply = await taskManager.watch('citychat', `小城对话 · ${friendName}`, async () => {
                const { callAIWithMessages } = await import('../aiService.js');
                const sceneEnv = (() => { const pl = PLACES.find(p => p.name === placeName); return pl ? placeAmbience(pl, hour) : `${placeName}里，一切照常`; })();
                return await callAIWithMessages({
                    systemPrompt: groupMode
                        ...
                        '3. 角色有自己的性格和关系网，自然地和其他角色互动；可以互相搭话、调侃、聊自己的事。\n' +
                        '4. 角色有自己的信息视角，各个角色对其他角色的熟知度也会有区别，根据角色间的关系和给到的认知信息进行对话；没有给出的关系、对方真实身份、未参与时的对话，不得假设或编造。新加入的角色不知道加入前的对话，若相关就让TA自然询问或保持不知情。\n' +
                        '5. 回应简短（每人20~60字）。\n' +
                        '6. 若有人提出约定或加好友：同意 → 在该角色那行末尾加标记。约定：【约定】HH:MM地点（临时）或【约定】HH:MM地点【永久】（表示该角色自己会按时赴约，谁说的约定就属于谁）；加好友：【加好友@对方游戏名】（@后必须是在场者的确切游戏名，群聊内任意角色可互相加，也可加' + promptMyName + '）；拒绝 → 自然拒绝，不加标记。\n' +
                    ...
                    userContent: `${myInfo}\n\n` +
                        (groupMode
                            ? `【在场者】\n${rosterText || '（只有你们两人）'}\n\n`
                            : (fp ? `【你】小城名：${esc(fp.name)}（${esc(jobDisplay(fp))}）\n性格：${esc((fp.aiProfile?.traits || []).join('、'))}\n你的当前状态：${statusNow(friendId, hour) || '空闲'}\n今日日程：${(fp.schedule || []).map(s => `${esc(s.time)} ${esc(s.place)}${s.act ? ' ' + esc(s.act) : ''}`).join('；') || '（暂无安排）'}\n` : `【你】${esc(friendName)}\n（未入住小城，作为路人回应）\n`)) +
                        (isRemote
                            ? `【此刻】${hour}点 · 远程消息：你正拿着手机回${promptMyName}的消息${nowStatus ? '（你此刻' + nowStatus + '）' : ''}\n\n`
                            : `此刻：${placeName}（${hour}点），${sceneEnv}。你正在「${friendAct || ('在' + placeName + '待着')}」\n\n`) +
                        `【对话历史】\n${history || '（刚开始聊）'}\n\n` +
