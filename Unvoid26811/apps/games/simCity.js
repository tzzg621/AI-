...
import {
    ...
    getSimCityWorld, saveSimCityWorld          // ★ 新增：世界状态
...
async function aiEvaluateProfile(roleId, profile, suggestion = '') {
    ...
    const raw = await callAIWithMessages({
        systemPrompt: '你是"模拟小城"的入住评估官。根据给定角色的设定，输出三部分：' +
            ...
            '只输出JSON：{"job":"school-teach-teacher","comment":"评语","traits":["标签"],"rest":"23:00~07:00","custom":{"擅长写作":true},"schedule":[{"time":"08:00","place":"杂货店","act":"整理货架"}]}，不要任何其他文字。',
...
function renderMain(container, globalState, onBack, roleId, profile) {
    ...
    const pageCount = Math.ceil(MAP_PLACES.length / 9);   // ★ 地图页数（指示器用）
    container.innerHTML = `
...
function careerWorkHere(profile, placeKey, subKey) {
    const j = profile.jobKey && JOB_DEFS[profile.jobKey];
    if (!j) return null;
...
async function doCareerWork(container, globalState, onBack, roleId, profile, jobDef, afterRender) {
    ...
    if (after > before) {
        ...
        await saveProfile(profile, roleId);
        await addMemento(roleId, profile, { text: `在${jobDef.name}晋升到 Lv${lvl}，获得奖励 +30 金币`, time: Date.now() });
...
function showCharSheet(container, globalState, onBack, roleId, profile) {
    ...
    const careerEntries = Object.entries(profile.career || {})
        .map(([k, exp]) => ({ key: k, job: JOB_DEFS[k], exp: exp || 0 }))
