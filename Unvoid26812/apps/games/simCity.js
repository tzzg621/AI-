...
import {
    ...
    getPresentAt, chatPairKey, getChatMessages, saveChatMessage, getAllChats, deleteChatMessages, deleteTempChats,
    getAllProfiles, getGroupChatMessages, getGroupRegistry, saveGroupRegistry,
    getSimCityWorld, saveSimCityWorld, getSimCityEstates, saveSimCityEstates          // ★ 新增：世界状态
...
async function aiEvaluateProfile(roleId, profile, suggestion = '') {
    ...
    // ★ 该角色可见的小城地点（静态全部 + 建成且可见的动态地产）
    const visiblePlaces = [
        ...PLACES.filter(p => !p.parent).map(p => p.name),                                  // 主地点
        ...PLACES.filter(p => p.parent).map(p => p.name),                                   // 子地点
        ...(simCityEstates?.estates || []).filter(e => e.status === 'built' && canSeeEstate(roleId, profile, e)).map(e => e.name)  // 可见建成地产
    ];
    // ★ 可入职职位（静态 + 可见动态，带地点/薪资）
    const visibleJobs = [
        ...Object.keys(JOB_DEFS).map(k => `${JOB_DEFS[k].name}（${JOB_DEFS[k].placeKey}${JOB_DEFS[k].subKey ? '·' + JOB_DEFS[k].subKey : ''}，时薪${JOB_DEFS[k].hourly}）`),
        ...(simCityEstates?.estates || []).filter(e => e.status === 'built' && canSeeEstate(roleId, profile, e)).flatMap(e => (e.jobs || []).map(j => `${j.name}（${e.name}${j.subKey ? '·' + j.subKey : ''}，时薪${j.hourly}）`))
    ];
    ...
    const raw = await callAIWithMessages({
        systemPrompt: '你是"模拟小城"的入住评估官。根据给定角色的设定，输出三部分：' +
            ...
            '★ 地点只能从【小城地点】中选择（未列出的地点在小城不存在）：【小城地点】' + visiblePlaces.join('、') + '\n' +
            ...
            '只输出JSON：{"job":"school-teach-teacher","comment":"评语","traits":["标签"],"rest":"23:00~07:00","custom":{"擅长写作":true},"ip":["fate"],"schedule":[{"time":"08:00","place":"杂货店","act":"整理货架"}]}，不要任何其他文字。',
...
async function assessEstateGoal(roleId, profile, goal, estateList) {
    ...
    const userContent =
        ...
        `已有地产：${JSON.stringify((estateList || []).map(e => ({ id: e.id, goal: e.goal, status: e.status, contributors: e.contributors })))}。\n` +
    ...
    try {
        const verdict = await taskManager.submit('city_eval', `🏗️ ${estate.goal} 焕新评估`, async () => {
            ...
            const systemPrompt =
                ...
                '5) jobs：相应职业数组，每个含 name、desc、requireSkills（所需技能数组，为未来职业系统预留，如["魔术"]），1~3个。' +
        ...
        // ★ 应用建成形态（同一地产对象"焕新"）
        estate.status = 'built';
        estate.name = String(verdict.name);
        ...
        estate.jobs = (Array.isArray(verdict.jobs) ? verdict.jobs : []).map((j, i) => ({
            key: 'est_job_' + estate.id + '_' + i,
            name: String(j.name || '职业' + (i + 1)),
            desc: String(j.desc || ''),
            requireSkills: Array.isArray(j.requireSkills) ? j.requireSkills : [],
            placeKey: estate.id,                                    // ★ 地产即上班地点（动态）
            subKey: '',                                             // 子地点职业后续
            base: Math.round(20 + estate.maxProgress / 200),        // 底薪按规模
            hourly: Math.round(5 + estate.maxProgress / 500),       // 时薪按规模
            quota: 2 + Math.floor(estate.maxProgress / 500)         // 岗位数按规模
        }));
        await saveEstates();
        ...
        <div class="simcity-root">
            ...
            <div class="simcity-body">
                ...
                <div class="simcity-stats">
                    <div class="simcity-stat"><div class="stat-ico">💰</div><div class="stat-v">${profile.money}</div><div class="stat-k">金钱</div></div>
                    <div class="simcity-stat"><div class="stat-ico">⚡</div><div class="stat-bar" style="--p:${Math.max(0, Math.min(100, profile.energy))}%"><i></i></div><div class="stat-k">体力 ${profile.energy}</div></div>
                    <div class="simcity-stat"><div class="stat-ico">😊</div><div class="stat-bar mood" style="--p:${Math.max(0, Math.min(100, profile.mood))}%"><i></i></div><div class="stat-k">心情 ${profile.mood}</div></div>
                </div>
...
async function doAction(container, globalState, onBack, roleId, profile, act, afterRender) {
    ...
    await save();
    // ★ 行动后小概率随机事件（世界惊喜感）
    const simCityEvt = simCityRandomEvent();
    if (simCityEvt) {
...
function renderPlace(container, globalState, onBack, roleId, profile, placeKey) {
    ...
    container.innerHTML = `
        ...
        return `
            ...
            <div class="simcity-actions">
                ${careerJob
            ? `<button class="simcity-btn primary" id="careerActBtn" style="${open ? '' : 'opacity:0.5;'}">${jobIcon(careerJob.key)} ${esc(careerJob.name)} · 上班${open ? '' : '（已打烊）'}</button>`
            : `<button class="simcity-btn primary" id="placeActBtn" style="${open ? '' : 'opacity:0.5;'}">${place.icon} ${esc(place.desc)}${open ? '' : '（已打烊）'}</button>`}
                ${(['mall', 'entertain'].includes(place.key) && (hour >= 19 || hour < 5)) ? `<button class="simcity-btn" id="placeNightBtn">🌙 夜生活事件</button>` : ''}
            </div>
        </div>`;
    ...
    container.innerHTML = `
        <div class="simcity-root">
            ...
            : `<button class="simcity-btn primary" id="subActBtn" style="${open ? '' : 'opacity:0.5;'}">${sub.icon} ${esc(sub.desc)}${open ? '' : '（已打烊）'}</button>`}
                ${(NIGHT_SPOTS.includes(sub.key) && (hour >= 19 || hour < 5) && venueOpen(sub.key, hour)) ? `<button class="simcity-btn" id="nightEventBtn">🌙 夜生活事件</button>` : ''}
            </div>
        </div>`;
...
function getPlace(placeKey) {
    ...
    for (const est of (simCityEstates?.estates || [])) {
        ...
        if (s) return { key: s.key, name: s.name, icon: s.icon || '🏛️', act: s.act || 'rest', desc: s.desc || '', parent: est.id, subs: [] };
    }
    return null;
}

// ★ 动态职业：JOB_DEFS 找不到时，查建成地产的职业（正式职位结构，可入职/发工资/上班）
...
function simCityRandomEvent() {
    if (Math.random() >= 0.14) return null;
    return SIMCITY_RANDOM_EVENTS[Math.floor(Math.random() * SIMCITY_RANDOM_EVENTS.length)];
}
    ...
    async function send(text) {
        ...
        if (m) {
            ...
            if (place && place.name !== placeName) {
                ...
                if (tm) hour = Math.min(23, parseInt(tm[1]));
        ...
        try {
            ...
            const reply = await taskManager.watch('citychat', `小城对话 · ${friendName}`, async () => {
                ...
                const sceneEnv = (() => {
                    ...
                    return uiEnvText(container, placeName) || placeAmbience(pl, hour);
                })();
                return await callAIWithMessages({
                    systemPrompt: isGroupChat()
                        ...
                        (isRemote
                            ? '2. 【重要】你现在正拿着手机和' + promptMyName + '聊天（' + hour + '点' + (nowStatus ? '，你此刻' + nowStatus : '') + '）。\n'
