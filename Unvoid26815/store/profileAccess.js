...
export function getVisibleProfile(targetId, viewerId, opts = {}) {
    ...
    // ★ L0 表象层：无条件可见（不认识也能看到"看起来的样子"）
    if (profile.L0) visible.extra['表象'] = toText(profile.L0);
    if (level >= 1 && profile.L1) visible.extra['熟人层'] = toText(profile.L1);
    if (level >= 2 && profile.L2) visible.extra['密友层'] = toText(profile.L2);
...
export function formatProfilePrompt(targetId, viewerId, targetName, opts = {}) {
    ...
    if (info.level === 0) {
        lines.push(info.extra['表象']
            ? '（以上只是表面印象，真实情况你还不了解）'
            : '（你们还不熟，以上仅为公开可见信息，请勿臆断更多）');
    }
    return lines.join('\n');
}
...
export function parseProfileText(text = '') {
    const chunks = { L0: [], L1: [], L2: [], L3: [], manual: [] };
    let section = null;
    for (const raw of text.split('\n')) {
        const line = raw.trim();
        if (!line) continue;
        if (line.startsWith('【L0】')) { section = 'L0'; continue; }
