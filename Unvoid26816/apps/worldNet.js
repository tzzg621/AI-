...
function createNPCCardHTML(npc, index, isConverted) {
    // ★ 已归档 NPC：灰卡显示，不可编辑、不可转化
    if (npc.archived) {
        return `
            <div class="world-entry" data-npc-index="${index}"
                 style="display:flex; flex-direction:column; gap:8px; opacity:0.55; filter:grayscale(0.9);">
                <div style="display:flex; align-items:center; gap:10px;">
                    <div class="worldnet-portrait-disabled" data-npc-id="${npc.id}"
                         style="width:48px; height:48px; border-radius:12px; overflow:hidden; flex-shrink:0; background:#e0e0e0;">
                        ${getPortraitHtml(npc.id)}
                    </div>
                    <div>
                        <div style="font-weight:600;">${esc(npc.base.name)}</div>
                        <div style="font-size:13px; color:#666;">${esc(npc.base.desc)}</div>
                        <div style="font-size:12px; color:#999; margin-top:2px;">📦 已归档 · 不可转化 / 不可编辑</div>
                    </div>
                </div>
                <button disabled
                ...
                <button class="npc-delete-btn" data-npc-index="${index}"
                        style="padding:8px; border-radius:10px; border:1px solid #e53935; background:white; color:#e53935; cursor:pointer; font-size:12px;">
                    🗑️ 删除
                </button>
            </div>
        `;
...
<div class="worldnet-portrait" data-npc-id="${npc.id}" style="width:48px; height:48px; border-radius:12px; overflow:hidden; flex-shrink:0; background:#e0e0e0; cursor:pointer;">
    ${getPortraitHtml(npc.id)}
                    ...
                    <div>
                        <div style="font-weight:600;">${esc(npc.base.name)}</div>
                        <div style="font-size:13px; color:#666;">${esc(npc.base.desc)}</div>
                        <div style="font-size:12px; color:#9c27b0; margin-top:2px;">🔒 ${esc(npc.flags.label || '特殊角色·不可转化')}</div>
                    </div>
                </div>
${!hasActiveChar ? `
...
` : `
    ...
    <button class="npc-archive-btn" data-npc-index="${index}" style="flex:1;padding:8px;border-radius:10px;border:1px solid #ff9800;background:white;color:#ff9800;cursor:pointer;font-size:12px;">📦 归档</button>
    <button class="npc-delete-btn" data-npc-index="${index}" style="flex:1;padding:8px;border-radius:10px;border:1px solid #e53935;background:white;color:#e53935;cursor:pointer;font-size:12px;">🗑️ 删除</button>
</div>
                ...
                <div class="page-card">
                    ...
                    <div style="background:#fff8e1; border-radius:12px; padding:12px; margin-bottom:16px;">
                        ...
                        <div style="display:flex; align-items:center; gap:10px; margin-bottom:8px;">
                            <span style="font-size:40px;">${esc(npc.base.emoji)}</span>
                            <div style="flex:1; font-size:18px; font-weight:600; word-break:break-word; overflow-y:auto; max-height:60px; line-height:1.3;">${esc(npc.base.name)}</div>
                        </div>
                        
                        <textarea rows="2" readonly
                                  style="width:100%; padding:8px; border-radius:8px; ${readOnlyStyle} font-size:13px; margin-bottom:8px;">${esc(npc.base.desc)}</textarea>
                        ...
                        <div style="margin-bottom:4px;">
                            ...
                            <div style="font-size:13px; background:#fff; padding:6px 10px; border-radius:8px; margin-top:2px;">${esc(npc.base.secret || '无')}</div>
                        </div>
                        <div style="margin-bottom:4px;">
                            <div style="font-size:13px; color:#666;">🗣️ 说话风格</div>
                            <div style="font-size:13px; background:#fff; padding:6px 10px; border-radius:8px; margin-top:2px;">${esc(npc.base.style || '无')}</div>
                        </div>
                    </div>
