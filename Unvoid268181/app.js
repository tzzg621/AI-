...
function renderHome() {
    ...
    document.documentElement.style.setProperty('--desk-bg', customBg ? `url('${customBg}')` : '');
    ...
    return `
<div class="home-screen theme-${currentTheme}${customBg ? ' has-custom-bg' : ''}">
        ...
        <div class="home-panels">
            <div class="home-panel" data-panel-index="0">
                <div class="home-grid">
                    ${primaryModules.map(renderAppCard).join('')}
                </div>
            </div>
            <div class="home-panel" data-panel-index="1">
        ...
        <div class="home-dot-indicator" aria-label="首页分页指示器">
            ${Array.from({ length: totalPanels }, (_, index) => `
                <button class="home-dot ${index === state.homePanelIndex ? 'active' : ''}" data-home-dot="${index}" aria-label="第 ${index + 1} 页" aria-current="${index === state.homePanelIndex ? 'true' : 'false'}"></button>
            `).join('')}
        </div>
    </div>
    `;
...
function bindHomeEvents() {
    ...
    if (homePanels) {
        homePanels.scrollTo({ left: homePanels.clientWidth * state.homePanelIndex, behavior: 'auto' });
        homePanels.addEventListener('scroll', () => {
            const index = Math.round(homePanels.scrollLeft / Math.max(homePanels.clientWidth, 1));
            state.homePanelIndex = Math.min(Math.max(index, 0), 1);
            homeDots.forEach((dot, dotIndex) => dot.classList.toggle('active', dotIndex === state.homePanelIndex));
        }, { passive: true });
    }

    homeDots.forEach((dot) => {
        dot.addEventListener('click', () => {
            const nextIndex = Number(dot.dataset.homeDot || 0);
            state.homePanelIndex = nextIndex;
            if (homePanels) {
                homePanels.scrollTo({ left: homePanels.clientWidth * nextIndex, behavior: 'smooth' });
            }
            homeDots.forEach((item, index) => item.classList.toggle('active', index === nextIndex));
        });
    });
    ...
    elements.pageContainer.querySelectorAll('.app-card[data-route]').forEach((button) => {
        button.addEventListener('click', () => {
            ...
            state.homePanelIndex = Number(panel?.dataset.panelIndex ?? 0);
