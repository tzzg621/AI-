...
export async function runTextAdventure(container, opts, resumeKey) {
    ...
    const callAI = async (action) => {
        ...
        const systemPrompt = '你是"模拟小城"的文游叙事引擎。基于【场景设定】与【玩家行动】推进剧情，追求故事性：\n' +
            ...
            '2) 每轮 scene 至少包含：环境细节 + 人物性格 + 情感反应，避免平铺直叙；\n' +
            '3) 剧情要有起伏：玩家行动 2~3 轮后安排一次"发现/意外/深度推进"（新线索、新角色出现、情绪高潮、事件推进），别让剧情原地打转；\n' +
            '4) 这是一场玩家的模拟游戏体验，注意当前的环境与体验感；\n' +
            '5) 选项要指向不同方向（现场调查/情感交流/暗中观察/直面风险/谨慎撤退等），而不只是性格倾向；\n' +
