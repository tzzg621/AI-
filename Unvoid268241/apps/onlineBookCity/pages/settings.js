...
import {
  loadBundle, importBundle, removeBundle,
...
export function bindEvents(container) {
  ...
  function renderStyleSection(container) {
    ...
    if (!bundle) {
      headerEl.innerHTML = '<div style="font-size:13px;color:#999;">📄 未导入文风包，AI 生成将使用基础写作指令</div>';
      dimListEl.innerHTML = '';
      lastPreviewText = '';
    ...
    const totalOptions = bundle.dimensions.reduce((s, d) => s + d.options.length, 0);

    headerEl.innerHTML = `
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
      <span style="font-size:14px;font-weight:600;">📄 ${esc(bundle.name || '未命名文风包')}</span>
      <span style="font-size:12px;color:#999;">${bundle.dimensions.length} 个维度 · ${totalOptions} 个选项</span>
    </div>
  `;

    const selections = getValidSelections(bundle);

    dimListEl.innerHTML = bundle.dimensions.map(dim => {
      const optionsHtml = dim.options.map(opt => {
        const active = selections[dim.name] === opt.name;
        return `
        <label class="obc-style-option"
               data-dim="${esc(dim.name)}" data-opt="${esc(opt.name)}"
               style="flex:0 0 auto;padding:8px 14px;border-radius:20px;border:2px solid ${active ? '#8e24aa' : '#e8e8e8'};
                      background:${active ? '#f3e5f5' : 'white'};cursor:pointer;font-size:13px;
                      color:${active ? '#8e24aa' : '#666'};font-weight:${active ? '600' : '400'};">
          ${esc(opt.name)}
        </label>
      `;
      }).join('');
  ...
  dimListEl.addEventListener('click', (e) => {
    ...
    const optName = el.dataset.opt;

    // 局部切换：同一维度内高亮新的，取消旧的
    dimListEl.querySelectorAll('.obc-style-option').forEach(o => {
      if (o.dataset.dim !== dimName) return;
      setOptionActive(o, o.dataset.opt === optName);
    });

    saveSelection(dimName, optName);
    updateStylePreview(container);
  });
  ...
  container.querySelector('#obcStyleImport').addEventListener('click', () => {
    ...
    input.addEventListener('change', async () => {
      ...
      renderStyleSection(container);
      await showAlert(`✅ 文风包「${bundle.name || '未命名'}」已导入（${bundle.dimensions.length} 个维度）`);
    });
    input.click();
  });
  ...
  container.querySelector('#obcStyleClear').addEventListener('click', async () => {
    ...
    const confirmed = await showConfirm('确定清除当前文风包吗？');
    if (!confirmed) return;
    removeBundle();
    renderStyleSection(container);
  });
