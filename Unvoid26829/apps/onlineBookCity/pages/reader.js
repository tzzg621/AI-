...
import { getBook, putBook } from '../store.js';
  ...
  mountEl.innerHTML = `
    <div class="obc-reader">
      <div class="obc-reader-header">
        <div class="obc-reader-title">${esc(book.title)} · 第${currentIdx + 1}章 ${esc(chapter.title)}</div>
      </div>

      <div class="obc-reader-content" id="obcReaderContent">
        ${esc(chapter.content) || '<div style="color:#bbb;padding:40px 0;text-align:center;">（本章暂无内容）</div>'}
      </div>
