...
async function collectTextData() {
    const data = {};
    // ★ ① DataSync 管理的数据（从 cache 枚举——这是核心！）
    try {
    ...
    // ★ ② 真实 localStorage（非 managed 的配置类）
...
// ---- 导出 Aoi 数据（IndexedDB）----
async function collectAoiData() {
    const data = { memory: [], chatHistory: [] };

    // 读取 AoiMemory（Aoi 的私有记忆）
    try {
        ...
        data.memory = all;
    } catch (e) { console.warn('AoiMemory 读取失败:', e); }

    // 读取 CreatorChatHistory（聊天窗口历史）
    try {
        const db = await new Promise((resolve, reject) => {
        ...
        data.chatHistory = all;
    } catch (e) { console.warn('CreatorChatHistory 读取失败:', e); }
            ...
            <div class="screen-content">
                <div class="page-card">
                    <div style="font-size:14px; color:#666; margin-bottom:16px;">
                        ...
                        <br><span style="font-size:12px; color:#999;">⚠️ 导入会覆盖当前所有数据，完成后请稍等 2 秒再刷新页面。</span>
                    </div>
...
export function bindDataBackupEvents(container, onBack) {
    ...
    // 导出文字
        container.querySelector('#exportTextBtn')?.addEventListener('click', async () => { 
        const textData = await collectTextData();
        const backup = {
    ...
    container.querySelector('#exportAllBtn')?.addEventListener('click', async () => {
        ...
        try {
            const textData = await collectTextData();
            const images = await collectImageData();
            const aoiData = await collectAoiData();
            const backup = {
                version: BACKUP_VERSION,
                createdAt: new Date().toISOString(),
                type: 'all',
            ...
            showToast(`✅ 已导出 ${Object.keys(textData).length} 项文字 + ${Object.keys(images).length} 张图片 + ${aoiData.memory.length} 条 Aoi 记忆`, '#9c27b0');
        } catch (e) {
            showToast('❌ 导出失败: ' + e.message, '#c62828');
        } finally {
        ...
        try {
            ...
            overlay.querySelector('.import-confirm').onclick = async () => {
                ...
                try {
                    ...
                    if (backup.aoi) {
                        try {
                            ...
                            if (backup.aoi.memory && backup.aoi.memory.length > 0) {
                                ...
                                const store = tx.objectStore('entries');
                                store.clear();
                                for (const entry of backup.aoi.memory) {
                                    store.add(entry);
                                }
                            ...
                            if (backup.aoi.chatHistory && backup.aoi.chatHistory.length > 0) {
                                ...
                                store2.clear();
                                for (const msg of backup.aoi.chatHistory) {
                                    store2.add(msg);
                                }
                                await new Promise((resolve, reject) => {
                    ...
                    showToast(`✅ 导入完成！${lsCount} 项文字 + ${imgCount} 张图片 + ${aoiMemoryCount} 条 Aoi 记忆 + ${aoiChatCount} 条聊天记录，请刷新页面`, '#2e7d32');
