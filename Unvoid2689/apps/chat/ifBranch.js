...
async function buildContextPrompt(branch) {
    ...
    for (const [id, name] of [[branch.charAId, branch.charA], [branch.charBId, branch.charB]]) {
        ...
        // ★ 角色自身设定（性别锚点）
        if (charData?.base) {
            parts.push(`【${name} 的角色设定】\n` + [
                `性别：${charData.base.gender || '未知'}`,
                `年龄：${charData.base.age || '未知'}`,
                `人设：${charData.base.desc || ''}`,
                `说话风格：${charData.base.style || ''}`,
                charData.base.detail ? `详细设定：${charData.base.detail}` : ''
            ].filter(Boolean).join('\n'));
        }
        ...
        try {
            ...
            const pKnow = formatProfilePrompt(otherId, id, otherName);
            if (pKnow) parts.push(pKnow);
        } catch (e) { /* 忽略 */ }
