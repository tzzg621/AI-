...
export class AoiMemory {
    ...
    async _save(incremental = true) {
        const db = await this._openDB();
        const tx = db.transaction(STORE_NAME, 'readwrite');
        const store = tx.objectStore(STORE_NAME);

        if (incremental && this._pending.length) {
            // ★ 增量：只 add 新条目（keyPath 自增 id，add 不会重复插入）
            for (const entry of this._pending) store.add(entry);
            this._pending = [];
        } else {
            // 全量（compress 等删除旧条目的场景才走）
            store.clear();
            for (const entry of this._entries) store.add(entry);
        }
    ...
    async record(type, data) {
        const entry = { ...data, _type: type, _timestamp: Date.now() };
    ...
    async storeFile(fileName, content) {
        const entry = { fileName, content, _type: 'file', _timestamp: Date.now() };
        this._entries.push(entry);
        this._pending.push(entry);          // ★
        await this._save(true);
        return entry;
