...
function renderBoardCell(app, session, seat, mine) {
    ...
    const who = `${seat.seat} 号 ${seat.name || ''}`;
    // 两个角标一律**常驻**（没内容时 CSS 自己藏），这样就地更新只改文字、不用管增删。
    // 位置的分工写在 werewolf.css 的 .ww-mini-badge 那一块（右上身份 / 右下我贴的 / 左下留给金水银水）
    return `
        <button class="ww-mini-seat ${cls}" data-board-seat="${seat.seat}" ${canMark ? '' : 'disabled'}
...
/**
 * 轮到我上警表态：两枚芯片（上警 / 不上警）。
 * **刻意不给进度、也不列别人举没举**：全桌一个一个表态、收齐了才公布名单，
 * 界面先说出去就等于把「后决定的人可以照着改主意」这扇门打开了（引擎那边同理，见 applySheriffSignup）。
 */
function renderDeclareRow(session) {
    return `
        <footer class="ww-bottom column">
 ...
 */
function declareTurn(app, close, seatNo, type) {
    return runTurn(app, close, {
 ...
 * 表态本身不进流水，所以这里不给 toast——问一句「你要不要上警」再自己回答一遍很怪；
