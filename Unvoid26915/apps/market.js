...
function renderMarketList() {
    const apps = [
        ...
        { id: 'txt-importer', text: '📄 TXT 导入工具', desc: '将文本文件转换为角色卡与世界书' },
        { id: 'rolecard-generator', text: '🎴 角色卡生成器', desc: 'AI 描述或文件导入生成角色卡' },
...
export function bindEvents(container) {
    if (viewingApp === 'pixel-avatar') {
        // 动态加载像素头像模块并初始化
        importWithRetry(() => import('./market/pixelAvatar.js')).then(mod => {
    ...
    if (viewingApp === 'ai-image-gen') {
        importWithRetry(() => import('./market/aiImageGen.js')).then(mod => {
            const appContainer = container.closest('.page-container') || container;
            appContainer.innerHTML = mod.render();
            mod.bindEvents(appContainer);
        }).catch(e => {
    ...
    if (viewingApp === 'txt-importer') {
        importWithRetry(() => import('./market/txtImporter.js')).then(mod => {
            const appContainer = container.closest('.page-container') || container;
            appContainer.innerHTML = mod.render();
            mod.bindEvents(appContainer);
        }).catch(e => {
            console.error('TXT 导入模块加载失败:', e);
            const appContainer = container.closest('.page-container') || container;
            appContainer.innerHTML = `<div class="screen-page"><div class="screen-content"><div class="page-card"><p style="text-align:center;color:#e53935;padding:40px;">❌ 模块加载失败（${e.message}）</p></div></div></div>`;
        });
        return;
    ...
    if (viewingApp === 'rolecard-generator') {
        importWithRetry(() => import('./market/roleCardGenerator.js')).then(mod => {
            const appContainer = container.closest('.page-container') || container;
            appContainer.innerHTML = mod.render();
            mod.bindEvents(appContainer);
        }).catch(e => {
            console.error('角色卡生成器加载失败:', e);
            const appContainer = container.closest('.page-container') || container;
            appContainer.innerHTML = `<div class="screen-page"><div class="screen-content"><div class="page-card"><p style="text-align:center;color:#e53935;padding:40px;">❌ 模块加载失败（${e.message}）</p></div></div></div>`;
        });
        return;
