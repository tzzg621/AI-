...
import { collectMoments, formatTime } from './momentsData.js';   // ★ 取数走数据层（与发现页同口径）
...
<div class="moments-list" style="flex:1;overflow-y:auto;padding:12px;">
    ...
    overlay.querySelector('#momentsInteractBtn').addEventListener('click', async () => {
        ...
        try {
            ...
            // ★ 一次调用批量生成（排除主视角：TA 的互动由用户自己做）
            const result = await generateMomentInteractionsBatch(recent, activeId);
    ...
    overlay.querySelector('.moments-list')?.addEventListener('click', async (e) => {
        ...
        if (aiReplyBtn) {
            ...
            btn.disabled = true; btn.textContent = '⏳';
            try {
                const list = await generateMomentReply(authorId, moment, activeId);
