...
loadHistory().then(saved => {
    if (saved.length > 0 && _messages.length === 0) { _messages = saved; }   // ★ 不覆盖新消息
});
...
export function bindEvents(container, options = {}) {
    ...
    fileInput?.addEventListener('change', async (e) => {
        ...
        for (const file of files) {
            if (file.size > MAX_SIZE) {
                appendMessage('agent', `❌ ${esc(file.name)} 文件过大（超过 500KB），已跳过`);
                hasError = true;
                continue;
            }
            try {
                ...
                appendMessage('agent', `❌ ${esc(file.name)} 读取失败: ${esc(e.message)}`);
                hasError = true;
            }
        }
    ...
    async function send() {
        ...
        ⚡ ${result.usage.total_tokens} tokens（输入 ${result.usage.prompt_tokens} / 输出 ${result.usage.completion_tokens}）
            ...
            loadingEl.remove();
            appendMessage('agent', '❌ ' + esc(e.message));
        }
    }
