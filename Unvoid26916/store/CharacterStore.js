...
export class CharacterStore {
    ...
    _load() {
        ...
        if (saved) {
            try {
                ...
                if (!parsed.signature) parsed.signature = '';         // ★ 个性签名（角色的动态数据）
                if (!parsed.signatureLog) parsed.signatureLog = [];   // ★ 签名更新日志（内容与结果绑定）
                return parsed;
            } catch (e) {
    ...
    // ---- 个性签名（角色的动态数据：自己写的情绪窗口，业务规则见 apps/signature.js）----
    getSignature() {
        return this.data.signature || '';
    }

    getSignatureLog() {
        return this.data.signatureLog || [];
    }

    // 原子写入：当前签名与日志一次落盘（ok=false 时只记拒绝，不动签名）
    // 频控计数从日志派生（不存独立计数器）→ 内容与结果永远绑定，不存在"计数与记录不同步"
    commitSignature(text, { ok = true, reason = '', source = 'chat' } = {}) {
        if (ok) this.data.signature = text;
        if (!this.data.signatureLog) this.data.signatureLog = [];
        const entry = { at: Date.now(), text, ok, source };
        if (reason) entry.reason = reason;
        this.data.signatureLog.push(entry);
        // 整体裁剪：记录与原因一同进出，不留半截
        if (this.data.signatureLog.length > 30) {
            this.data.signatureLog = this.data.signatureLog.slice(-30);
        }
        this._save();
        return ok;
    }
