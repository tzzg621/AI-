 ...
 * @param {string} [excludeId] 不参与 AI 互动的角色（主视角）
 * @returns {Promise<Array<{friendId, action:'reply'|'comment', targetCommentId?, text}>>}
 */
export async function generateMomentReply(authorId, moment, excludeId = null) {
    ...
    const friendIds = (authorStore.getFriendIds() || [])
        ...
        .filter(id => id !== excludeId)          // ★ 去用户化：主视角的赞/评论只能由用户自己做，AI 不代劳
 ...
 */
export async function generateMomentInteractionsBatch(recent, excludeId = null) {
    const blocks = [];
