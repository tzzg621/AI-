...
export function bindEvents(container) {
    container.querySelectorAll('.settings-section-item[data-section="api"]').forEach(item => {
        item.addEventListener('click', async () => {
            let apiModule;
            try {
                apiModule = await importWithRetry(() => import('./settingsApi.js'));
            } catch (e) {
    ...
    container.querySelectorAll('.settings-section-item[data-section="image-gen"]').forEach(item => {
        item.addEventListener('click', async () => {
            let imgGenModule;
            try {
                imgGenModule = await importWithRetry(() => import('./settingsImageGen.js'));
            } catch (e) {
                alert('生图设置模块加载失败');
    ...
    container.querySelectorAll('.settings-section-item[data-section="desktop-theme"]').forEach(item => {
        item.addEventListener('click', async () => {
            let themeModule;
            try {
                themeModule = await importWithRetry(() => import('./settingsDesktopTheme.js'));
            } catch (e) {
                alert('桌面主题模块加载失败');
        ...
        item.addEventListener('click', async () => {
            let backupModule;
            try {
                backupModule = await importWithRetry(() => import('./settingsDataBackup.js'));
            } catch (e) {
                alert('数据备份模块加载失败');
        ...
        item.addEventListener('click', async () => {
            let globalModule;
            try {
                globalModule = await importWithRetry(() => import('./settingsGlobal.js'));
            } catch (e) {
                alert('全局设置模块加载失败');
