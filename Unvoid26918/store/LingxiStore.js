...
// ---- pair 快照内存缓存（懒加载）----
// 只有 getPairTopic 实际读取时才入缓存；
// savePairTopicSnapshot 写库成功后失效对应条目；
// 容量超限时按插入顺序淘汰最旧条目（FIFO）。
const pairTopicCache = new Map();
const MAX_CACHE_ENTRIES = 50;

function cachePairTopic(id, record) {
    if (!record) return;               // 空结果不缓存，避免假阴性残留
    pairTopicCache.set(id, record);
    if (pairTopicCache.size > MAX_CACHE_ENTRIES) {
        const oldestKey = pairTopicCache.keys().next().value;
        pairTopicCache.delete(oldestKey);
    }
}
...
function normalizeResponse(response, topicId) {
    ...
    return {
        ...
        text,
        topicName: normalizeTopicName(response?.topicName),
...
function normalizePairSnapshot(record) {
    ...
    return {
        ...
        topicSnapshot: {
            ...
            authorId: String(record?.topicSnapshot?.authorId || '')
        },
        responseSnapshots,
...
export async function savePairTopicSnapshot(record) {
    ...
    try {
        ...
        // ★ 写库成功后让缓存失效，下次读取重新入库
        pairTopicCache.delete(normalized.id);
...
export async function getPairTopic(pairKey, topicId) {
    ...
    // ★ 命中缓存直接返回
    if (pairTopicCache.has(id)) return pairTopicCache.get(id);
...
export function projectPairTopic(snapshot, viewerId, getName) {
    const responses = Array.isArray(snapshot?.responseSnapshots)
        ? snapshot.responseSnapshots
        : [];

    const ownResponse = responses.find(
        response => response.authorId === viewerId
    );
    ...
    return {
        topicName:
            ownResponse?.topicName ||
            fallbackResponse?.topicName ||
            '',

        topicText: snapshot?.topicSnapshot?.text || '',

        isOwnPost:
            Boolean(viewerId) &&
            snapshot?.topicSnapshot?.authorId === viewerId,

        responses: responses.map(response => {
            const isOwn = response.authorId === viewerId;

            return {
                isOwn,
                label: isOwn
                    ? `${getName(viewerId) || viewerId}在该话题下的回复`
                    : `${response.anonymousLabel || '匿名'}的回复`,
                text: response.text
            };
        })
    };
