...
import { unbindCharPresetsOf } from './roleData.js';
...
function getCachedModels() {
    try {
        // ★ 键名带上当前编辑的预设 ID
        const saved = localStorage.getItem('ai_models_cache_' + editingPresetId);
...
function setCachedModels(models) {
    // ★ 保存到当前预设专用的键
    localStorage.setItem('ai_models_cache_' + editingPresetId, JSON.stringify(models));
}
                    ...
                    <div style="border:1px solid #eee; border-radius:12px; padding:16px;">
                        ...
                        <div style="margin-bottom:10px;">
                            ...
                            <div style="display:flex; gap:6px;">
                                <select id="aiModel" style="flex:1; min-width:0; border:1px solid #ccc; border-radius:6px; padding:6px 8px; font-size:13px;">
    ${(() => {
            // ★ 确保当前预设的模型名一定在选项中
            const allModels = cachedModels.length > 0
                ? (current.model && !cachedModels.includes(current.model)
                    ? [current.model, ...cachedModels]
                    : cachedModels)
                : (current.model ? [current.model] : []);

            if (allModels.length === 0) {
                return `<option value="" selected>请刷新</option>`;
            }
            return allModels.map(m =>
                `<option value="${m}" ${current.model === m ? 'selected' : ''}>${m}</option>`
            ).join('');
        })()}
</select>
...
export function bindApiEvents(container, onBack) {
    ...
    // 保存
    container.querySelector('#savePresetBtn')?.addEventListener('click', async () => {
        const presets = getPresets();
        ...
        savePresets(presets);
        // 保存成功后清空模型缓存
        localStorage.removeItem('ai_models_cache_' + editingPresetId);
        const { showAlert } = await import('../store/dialog.js').catch(() => ({ showAlert: null }));
        if (showAlert) showAlert(`✅ 预设「${presets[idx].name}」已保存`);
        else alert(`✅ 预设「${presets[idx].name}」已保存`);

    });

    // 设为默认
    container.querySelector('#setDefaultPresetBtn')?.addEventListener('click', async () => {
        const presets = getPresets();
        presets.forEach(p => p.isDefault = (p.id === editingPresetId));
        savePresets(presets);
        const { showAlert } = await import('../store/dialog.js').catch(() => ({ showAlert: null }));
        if (showAlert) showAlert('✅ 已设为默认预设');
        else alert('✅ 已设为默认预设');
    ...
    // 删除
    container.querySelector('#deletePresetBtn')?.addEventListener('click', async () => {
        const { showConfirm } = await import('../store/dialog.js').catch(() => ({ showConfirm: null }));
        let confirmed;
        if (showConfirm) confirmed = await showConfirm('确定要删除这个预设吗？');
        else confirmed = confirm('确定要删除这个预设吗？');
        if (!confirmed) return;

        const removedId = editingPresetId;
        let presets = getPresets();
        presets = presets.filter(p => p.id !== removedId);
        if (presets.length === 0) { alert('至少需要保留一个预设'); return; }
        if (!presets.some(p => p.isDefault)) presets[0].isDefault = true;
        savePresets(presets);
        unbindCharPresetsOf(removedId);   // 这条预设没了，绑着它的角色一起解绑（不留悬空引用）
    ...
    container.querySelector('#refreshModelsBtn')?.addEventListener('click', async () => {
        ...
        if (!endpoint) {
            const { showAlert } = await import('../store/dialog.js').catch(() => ({ showAlert: null }));
            if (showAlert) showAlert('请先填写 API 地址');
            else alert('请先填写 API 地址');
            return;
        }
        if (!apiKey) {
            const { showAlert } = await import('../store/dialog.js').catch(() => ({ showAlert: null }));
            if (showAlert) showAlert('请先填写 API 密钥');
            else alert('请先填写 API 密钥');
            return;
        }
        ...
        try {
            ...
            // ★ 确保当前选中的模型名始终在列表中
            const mergedModels = currentVal && !models.includes(currentVal)
                ? [currentVal, ...models]
                : models;
            selectEl.innerHTML = mergedModels.map(m =>
