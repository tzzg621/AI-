 ...
 */
export async function callStoryAI({ systemPrompt, userContent, label, maxTokens = DEFAULT_MAX_TOKENS }) {
    if (!hasKey()) throw new Error('还没有配置 AI，先去设置里填一条 API');
    return taskManager.watch('textgame', label, async () => {
        const raw = await withTimeout(
            callAIWithMessages({ systemPrompt, userContent, maxTokens, temperature: 0.9 }),
            DEFAULT_TIMEOUT
        );
