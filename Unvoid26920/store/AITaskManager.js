...
//
// ⚠️ label 是外部文本（各模块把剧本名/角色名/提示词前几十字塞进来），
//    渲染一律走 esc()——它是**唯一**的注入面（面板 innerHTML，且 display:none 也照样解析）。
        ...
        if (running.length > 0) {
            html += running.map(t => `
                <div style="display:flex; align-items:center; gap:8px; padding:8px 0; border-bottom:1px solid #f5f5f5;">
                    <span>⏳</span>
                    <span style="flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${esc(t.label)}</span>
            ...
            completed.slice(0, MAX_COMPLETED).forEach(t => {
                html += `
                    <div style="display:flex; align-items:center; gap:8px; padding:6px 0;">
                        ...
                        <span style="flex:1; color:#888; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${esc(t.label)}</span>
                ...
                html += `
                    <div style="display:flex; align-items:center; gap:8px; padding:6px 0;">
                        ...
                        <span style="flex:1; color:#bbb; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                            ${esc(t.label)}
                            <span style="font-size:11px; color:#ddd; margin-left:4px;">${esc(t.error || '')}</span>
                        </span>
