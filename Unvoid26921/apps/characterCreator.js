...
export function createCharacterFromNpc(npcData, {
    ...
    const id = npcData.id;
    const charData = createDefaultCharacterData(id, {
        name: npcData.base?.name || '未知角色',
        emoji: npcData.base?.emoji || '❓',
        desc: (npcData.base?.desc || '') + (additionalDesc ? `\n${additionalDesc}` : ''),
        detail: npcData.base?.detail || '',            // ★ 补
        gender: npcData.base?.gender || '未知',         // ★ 补
        age: npcData.base?.age || '未知',               // ★ 补
        orientation: npcData.base?.orientation || '未知', // ★ 补
...
请按以下 JSON 格式返回，不要加 markdown 标记：
{
    ...
    "style": "说话风格描述",
    "relations": [
        {"name": "法师", "relation": "旧友", "perspective": "年轻时一起冒险，他救过我的命"}
    ],
    "profile": {
        "L0": "第一眼看到的表象：表面性别、长相、整体气质（一段纯文本，可以刻意与真实不符）",
        "L1": "好友能看到的性格细节（一段纯文本）",
