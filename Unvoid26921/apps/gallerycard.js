...
function renderProfileSection(c) {
    ...
    if (!l0Text && !l1Text && !l2Text && !l3Text && !manualText) return '';
    ...
    return `
        <div style="background:#f3e5f5; border-radius:12px; padding:12px; margin-bottom:12px;">
            ...
            ${l0Text ? `<div style="font-size:12px; color:#7b1fa2; margin-bottom:2px;">【L0 表象层】</div><div style="font-size:13px; color:#6a1b9a; white-space:pre-wrap; margin-bottom:8px;">${esc(l0Text)}</div>` : ''}
...
<textarea id="rc-profile-l0" rows="2" placeholder="第一眼看到的表象：表面性别、长相、气质……"
    style="width:100%; border:1px solid #ddd; border-radius:8px; padding:8px 10px; font-size:12px; box-sizing:border-box; resize:vertical; margin-bottom:10px;">${esc((c.profile && typeof c.profile.L0 === 'string') ? c.profile.L0 : '')}</textarea>
