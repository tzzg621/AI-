...
/**
 * 清理发现页数据（删除所有 status='discover' 且未在发现页展示的书）
 * @returns {Promise<number>} 删除的数量
 */
export async function cleanupDiscoverBooks() {
  const db = await openDB();

  // ★ 收集所有被引用的书 ID（包括 pinned 和各个类别的 content）
  const referencedIds = new Set();

  // 1. 从 pinned 收集
  const pinned = await getDiscoverData('pinned') || [];
  pinned.forEach(p => referencedIds.add(p.id));

  // 2. 从各个分类的 content 收集
  const cats = await getDiscoverData('categories') || [];
  for (const cat of cats) {
    if (cat.id === 'all') continue;
    const content = await getDiscoverData(`content_${cat.id}`);
    if (content) {
      (content.golden || []).forEach(id => referencedIds.add(id));
      (content.newWorks || []).forEach(id => referencedIds.add(id));
      (content.cards || []).forEach(id => referencedIds.add(id));
    }
  }

  // ★ 只删没有被引用的 discover 书
  const books = await getBooksByStatus('discover');
  const idsToDelete = books
    .filter(b => !referencedIds.has(b.id))
    .map(b => b.id);
 ...
 * 从 AI 返回数据创建并保存书籍
 * 所有书结构一致，只是填充程度不同
 * @param {object} aiData - AI 返回的 { golden, newWorks, cards }
 * @param {string} catId - 类别 ID
 * @param {string} catName - 类别名称
 * @returns {Promise<{golden: string[], newWorks: string[], cards: string[]}>} ID 引用
 */
export async function saveBooksFromAI(aiData, catId, catName) {
  const bookIds = { golden: [], newWorks: [], cards: [] };

  for (const type of ['golden', 'newWorks', 'cards']) {
    const items = aiData[type] || [];
