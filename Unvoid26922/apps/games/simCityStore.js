...
export async function getAllChats() {
    return new Promise((resolve) => {
        openDB().then((db) => {
            const tx = db.transaction(STORE_CHATS, 'readonly');
            const req = tx.objectStore(STORE_CHATS).openCursor();
            const result = [];
            req.onsuccess = () => {
                const c = req.result;
                if (c) {
                    // ★ 用 cursor.key 判断（getAll() 返回值不带 key，旧写法过滤恒失效）
                    if (!String(c.key || '').startsWith('__') && Array.isArray(c.value.messages)) result.push(c.value);
                    c.continue();
                } else resolve(
                    result
                        .sort((a, b) => a.convId === b.convId ? a.chunk - b.chunk : a.convId.localeCompare(b.convId))
    ...
    return new Promise((resolve) => {
        openDB().then((db) => {
            ...
            req.onsuccess = () => {
                ...
                if (String(c.key || '').startsWith('__') || !Array.isArray(c.value.messages)) { c.continue(); return; }
                const idx = c.value.messages.findIndex(m => m.id === id);
                if (idx >= 0) {
                    c.value.messages.splice(idx, 1);
                    if (c.value.messages.length) store.put(c.value, c.key);
                    else store.delete(c.key);
                    resolve(true);
                    return;
                }
                c.continue();
    ...
    return new Promise((resolve) => {
        openDB().then((db) => {
            ...
            req.onsuccess = () => {
                ...
                if (String(c.key || '').startsWith('__') || !Array.isArray(c.value.messages)) { c.continue(); return; }
                const before = c.value.messages.length;
                c.value.messages = c.value.messages.filter(m => !idSet.has(m.id));
                if (c.value.messages.length !== before) {
                    if (c.value.messages.length) store.put(c.value, c.key);
                    else store.delete(c.key);
                }
                c.continue();
...
// ---- 清理孤儿文游会话（sessionStorage 标记丢失 = 关页遗留；进小城时调用）----
export async function cleanupStaleAdvSessions() {
    const db = await openDB();
    return new Promise((resolve) => {
        const tx = db.transaction(STORE_ADV_SESSIONS, 'readwrite');
        const store = tx.objectStore(STORE_ADV_SESSIONS);
        const req = store.openCursor();
        let removed = 0;
        req.onsuccess = () => {
            const c = req.result;
            if (c) {
                let alive = false;
                try { alive = !!sessionStorage.getItem(c.key); } catch (e) { }   // 有任意标记值即存活（兼容旧 JSON 格式）
                if (!alive) { store.delete(c.key); removed++; }
                c.continue();
