 ...
 *
 * **这张表不是半自动的全部门槛**：上警表态（`day_sheriff_signup`）同属「全桌挨个拿主意、
 * 又没有话可读」，只是它收的不是票、也不该被算进票箱，所以另立 `pendingDeclarers` 那张名单，
 * 两张都读的那个落点在 werewolf.js 的 `pendingActors`。
 */
 ...
 *
 * 上警表态那一拍不在四张票箱里，走它自己的那张（`pendingDeclarers`，见下面竞选那一节）——
 * 两张都是「全桌还差谁没拿主意」，半自动那一边读的是同一对。
 */
 ...
 * 发牌开局（座位必须坐满）
 *
 * `claims` = **发牌之前就已经定下的身份**，`[{ seat, role }]` 按顺序兑（许愿水晶走的就是这条路，
 * 见 werewolf.js 的 askWish）。它是「从池子里预扣一份、钉给那一座」，不是洗完之后再换座位：
 * 差别在于「这份池子里没有了」这句话会出现在**兑许愿的那一刻**，而不是变成一个发牌之后的意外。
 * **减去的是「一份」不是「那个身份」**——狼人有 4 份，许掉一份池子里还剩 3 份照常发。
 * 兑不出来的许愿（池子里没这份 / 同一座许了两次 / 那个座号压根不在座）就地作废，那一座按普通
 * 座位参与洗牌：引擎拿到坏数据不抛错、也绝不把池子兑少（少一份就会有座位分不到牌）。
 *
 * @param {{rng?: Function, claims?: Array<{seat:number, role:string}>}} opts
 * @returns {boolean} 是否开成
 */
export function startGame(session, { rng = Math.random, claims = [] } = {}) {
    ...
    // 先兑许愿：兑成一条就从池子里拿走一份、并记住那一座要拿哪张
    const seated = new Set(seats.map(s => s.seat));
    const pinned = new Map();
    for (const c of claims || []) {
        const seatNo = Number(c?.seat);
        if (!seated.has(seatNo) || pinned.has(seatNo)) continue;
        const at = roles.indexOf(c?.role);
        if (at < 0) continue;
        roles.splice(at, 1);
        pinned.set(seatNo, c.role);
    }

    const dealt = shuffle(roles, rng);
    // 座位对象是复用的（同一张桌再来一局）：死因、翻牌这些**座位级**的上一局残留要一起清。
    // 许过愿的座位直接拿钉好的那张，其余按座号顺序从洗好的池子里一张张抽——兑许愿是一份换一座，
    // 所以池子里剩几张正好等于没许愿的座位数，抽不空也剩不下
    seats.forEach(s => {
        s.role = pinned.get(s.seat) || dealt.shift();
        s.alive = true; s.death = null; s.flipped = false;
    });
    ...
    session.wolfLog = [];
    session.witchLog = [];
    // 评分用的两本账（见 scoreGame）。它们记的不是「真相」而是「动作」：谁投了谁、谁开了枪。
    // 之所以非要另记，是因为原来的载体都会用完就没——票箱每轮清空（settleNight / startNextNightPhase），
    // 票投给谁只在 events 的文案里，而 events 还有 200 条的截断；「谁开的枪」则哪里都没结构化记过。
    session.voteLog = [];
    session.hunterLog = [];
...
export function settleNight(session, rng = Math.random) {
    ...
    // 这一枪要留账：`night` 下面几行就清空了，而死者身上只记「死于枪」、不记开枪的是谁。
    // 记的是**他冲谁开的枪**（不是「有没有把人带走」）——账本记动作、评分看阵营，
    // 与投票那条同口径（投给狼就算，不管票有没有把人投出去）。弃枪不记，照 wolfLog 空刀不记。
    if (shooter && victim) {
        session.hunterLog = [...(session.hunterLog || []), { round: session.round || 1, by: shooter.seat, target: victim.seat }];
    }
...
/**
 * 这一拍还有哪些人没表态（按座号顺序）。半自动按这张名单一座一座问 AI——
 * 与 `pendingVoters` 同款，只是那一张数的是票箱，这一张数的是 `sheriffSignup`
 * （**键在＝已经表过态**，上与不上都算，与 `applySheriffSignup` 一个口径）。
 * 不是上警表态那一拍就返回空数组（半自动靠这一条判断「这一拍要不要开工」）。
 */
export function pendingDeclarers(session) {
    if (session?.phase !== 'day_sheriff_signup') return [];
    const said = new Set(Object.keys(session?.sheriffSignup || {}).map(Number));
    return sheriffSignupSeats(session).filter(s => !said.has(s.seat));
}

/** 这一拍该谁表态：还没表过态的第一个人。带**阶段闸**（照 currentSheriffVoter 的写法） */
export function currentSheriffSignup(session) {
    return pendingDeclarers(session)[0] || null;
}
...
/**
 * 投票留账（评分用）：**开票那一刻**把这一轮的票抄进 `voteLog`，一条票一条记录。
 *
 * 非抄不可：票箱（`session.votes` / `session.pk.votes`）下面几步就清空了，而开票摊进
 * `events` 的那份只把「投给谁」写在文案里，何况 events 还有 200 条的截断。事后重建不出来。
 *
 * **去重键是 `(round, stage)` 而不是 `round`**：同一天可能开两次票（白天放逐、平票 PK），
 * 靠 round 分不开。而且 `settleVote` 自己没有阶段守卫（界面点「开票」两下都能进来），
 * 已记过的这一轮就整轮跳过——账本只增不改，重复调用是幂等的。
 *
 * **竞选票不进这本账**：`settleSheriff` / `settleSheriffPk` 不调这个函数。选的是警长不是放逐，
 * 本来就不该算「点对狼」。弃票照记（`target: null`）——那是「谁没投」的底账，评分时才滤掉。
 */
function logVotes(session, stage, votes) {
    const round = session.round || 1;
    if ((session.voteLog || []).some(e => e.round === round && e.stage === stage)) return;
    const rows = Object.entries(votes || {}).map(([by, target]) => ({ round, stage, by: Number(by), target: target ?? null }));
    if (rows.length) session.voteLog = [...(session.voteLog || []), ...rows];
}
...
export function settleVote(session) {
    ...
    logVotes(session, 'vote', session.votes);   // 票箱下面几步就被 startNextNightPhase / settleNight 清掉
...
export function settlePk(session) {
    ...
    const out = top.length === 1 ? top[0] : null;
    revealVotes(session, counts, votes);
    logVotes(session, 'pk', votes);             // 同上：session.pk 下一行就置 null 了
...
export function applyHunterShot(session, targetSeat) {
    ...
    if (targetSeat == null) {
        ...
        // 留账（同上：死者身上只记死因，不记开枪的是谁）。走到这里一定是真开了枪——弃枪在上面那一支
        session.hunterLog = [...(session.hunterLog || []), { round: session.round || 1, by: me.seat, target: target.seat }];
...
/* ---------------- 身份评分（结算时算一次；纯函数，账本进、分数出） ---------------- */

/**
 * 评分表——**这些分值是用户 2026-09-16 逐轮拍板的，别顺手调**：
 * 两张板子的狼都恰好占 1/3（2/6、4/12），而 ±4/−2 那一族的瞎打期望恰好是 0
 * （4×(1/3) + (−2)×(2/3) = 0）——盈亏平衡概率 = 1/3 = 狼的基础浓度，读牌比瞎猜准才赚。
 * 女巫的毒与猎人的枪用的是同一个公式，不是巧合。
 */
const SCORE = {
    win: 10,                        // 赢（好人赢、狼赢共通）
    vote: 2,                        // 投票投给狼（只有好人算）
    seer: 4,                        // 预言家验到狼（验到好人不加）
    guard: 3,                       // 守卫守中刀口（守错不扣）
    healGood: 2, healWolf: -2,      // 女巫救下好人 / 救下狼
    poisonWolf: 4, poisonGood: -2,  // 女巫毒死狼 / 毒死好人
    shotWolf: 4, shotGood: -2,      // 猎人打中狼 / 打中好人
    kill: 3                         // 狼每刀成功带走一个好人
};

/** 这一局有过夜间动作的那几夜（三本账里有任一条目的 round） */
function nightRoundsOf(session) {
    const rounds = new Set();
    for (const e of session.wolfLog || []) rounds.add(e.round);
    for (const e of session.guardLog || []) rounds.add(e.round);
    for (const e of session.witchLog || []) rounds.add(e.round);
    return [...rounds].sort((a, b) => a - b);
}

/**
 * 某一夜的「保没保住」——把 `settleNight` 里那几个局部量从两本账上重推出来。
 *
 * 引擎里 `protectedBy = (guarded?1:0) + (saved?1:0)`、规则是 `!== 1` 人就死。两半分别落在
 * `wolfLog[].guarded` 与 `witchLog[].saved` 上，而 `saved === true` 必然意味着
 * `witchLog.killed === night.wolfTarget`（`applyWitch` 只放行「救今晚刀口」，见那边的守卫）。
 * 所以这里推出来的与 `settleNight` 那份等价——**同守同救不必往 session 上加字段**。
 *
 * 同守同救（两边都使了劲，人反而没了）**守卫与女巫都不计分**，用户口径是
 * 「如果守卫加分，那女巫就得减分，没必要。这个本来就是守卫和女巫之间的默契」。
 * 但**狼照拿 +3**——人确实死了，狼不在这对关系里。
 */
function nightOutcomeOf(session, round) {
    const wolf = (session.wolfLog || []).find(e => e.round === round) || null;
    const guard = (session.guardLog || []).find(e => e.round === round) || null;
    const witch = (session.witchLog || []).find(e => e.round === round) || null;
    const guarded = !!(wolf && wolf.guarded === true && guard);
    const saved = !!witch?.saved;
    return {
        guarded, saved,
        protectedBy: (guarded ? 1 : 0) + (saved ? 1 : 0),
        died: !!wolf && ((guarded ? 1 : 0) + (saved ? 1 : 0)) !== 1,
        guardSeat: guarded ? guard.by : null,
        witch,
        bothBlamed: guarded && saved
    };
}

/**
 * 一局结束后各座位的身份评分（结算与战绩用），每座一行。
 *
 * **纯函数、零写入**：分数永远从账本重算，不存累加器——所以调多少次都是同一个结果。
 * 这是「切页面 / 退出重进不会多算」的结构性解法，比加锁或标志位可靠。
 *
 * 账本记的是**动作**（谁投了谁、谁冲谁开的枪），评分才看**阵营**——与用户定的
 * 「投票投给狼就算，不管这一票有没有把人投出去」同口径。
 *
 * 一处反直觉但故意的：**每一只狼都拿这 +3**。狼那一刀是狼队一起投出来的，`wolfLog`
 * 根本不记是谁提的刀（没有 `by`），所以「只给提刀的那只狼」在数据上就做不到；
 * 何况用户要的就是「想加分多，自然需要团队合作」——赢的 +10 也是全队每人都拿。
 *
 * @returns {Array<{seat:number, role:string, win:boolean, total:number, items:Array<{k:string,v:number}>}>}
 */
export function scoreGame(session) {
    const winner = session?.winner || null;
    const campOf = seatNo => (factionOf(seatAt(session, seatNo)?.role) === 'wolf' ? 'wolf' : 'good');

    return (session?.seats || []).map(s => {
        const role = s.role;
        const mine = factionOf(role);
        const items = [];
        const add = (k, v) => items.push({ k, v });

        if (winner && mine === winner) add('win', SCORE.win);

        // 投票投给狼：**只有好人算**。用户口径：狼没有减分项，投票与它们不相关——
        // 给狼的投票也计分等于逼它去投队友，会扭曲战术。弃票、投给好人都不计。
        // 竞选票不在这本账里（选的是警长不是放逐），所以这里不必再过滤。
        if (mine === 'good') {
            for (const e of session.voteLog || []) {
                if (e.by !== s.seat || (e.stage !== 'vote' && e.stage !== 'pk')) continue;
                if (e.target != null && campOf(e.target) === 'wolf') add('vote', SCORE.vote);
            }
        }

        for (const e of session.seerLog || []) {
            if (e.by === s.seat && e.isWolf) add('seer', SCORE.seer);
        }

        for (const e of session.hunterLog || []) {
            if (e.by === s.seat) add('shot', campOf(e.target) === 'wolf' ? SCORE.shotWolf : SCORE.shotGood);
        }

        if (role === 'werewolf' || role === 'guard' || role === 'witch') {
            for (const round of nightRoundsOf(session)) {
                const o = nightOutcomeOf(session, round);
                if (role === 'werewolf' && o.died) add('kill', SCORE.kill);
                if (role === 'guard' && o.guarded && !o.bothBlamed) add('guard', SCORE.guard);
                if (role === 'witch' && o.witch?.by === s.seat) {
                    // 同守同救时她不是「救下狼」而是白忙一场——不扣，也不加
                    if (o.saved && !o.bothBlamed) add('heal', campOf(o.witch.killed) === 'wolf' ? SCORE.healWolf : SCORE.healGood);
                    if (o.witch.poisoned != null) add('poison', campOf(o.witch.poisoned) === 'wolf' ? SCORE.poisonWolf : SCORE.poisonGood);
                }
            }
        }
        ...
        return {
            seat: s.seat, role,
            win: !!(winner && mine === winner),
            total: items.reduce((n, i) => n + i.v, 0),
            items
        };
    });
}
