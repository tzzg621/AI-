                ...
                <div class="page-card">
                    ...
                    <div class="memory-character-list">
                        ${characters.map(char => `
                            ...
                            ">
                                <span style="font-size:28px;">${esc(char.emoji)}</span>
                                <div style="flex:1;">
                                    <div style="font-weight:600; font-size:15px;">${esc(char.name)}</div>
...
function renderCharacterMemories(charId) {
    ...
    const info = store.getInfo();
    const chatMemories = store.getMemories();
    // ★ 尝试从角色名册获取名字和 emoji（优先用 base 中的）
    let charName = info.name;
    let charEmoji = info.emoji;
    try {
        const roleData = localStorage.getItem('rolebook_characters');
        if (roleData) {
            const characters = JSON.parse(roleData);
            ...
            if (found) {
                if (!charName) charName = found.base.name;
                if (!charEmoji) charEmoji = found.base.emoji;
            }
        }
    } catch (e) { /* 忽略 */ }
    ...
    return `
        <div class="screen-page">
            <div class="screen-header">
                <div class="screen-title">${esc(charEmoji)} ${esc(charName)}</div>
                <div class="header-spacer"></div>
            </div>
            <div class="screen-content">
                <div class="page-card">
                        ...
                        <div style="position:relative; padding-left:20px; border-left:2px solid #e0e0e0;">
                            ${allMemories.map(m => `
                                <div style="margin-bottom:16px; position:relative;">
                                    ...
                                    "></div>
                                    <div style="font-size:12px; color:#999; margin-bottom:2px;">${esc(m.time || '未知时间')}</div>
                                    <div style="font-size:14px; color:#333; line-height:1.5;">${esc(m.content || m.text || '')}</div>
                                </div>
                            `).join('')}
                        </div>
                    `}
