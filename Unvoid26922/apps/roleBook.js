...
import { clearImageCache, getPortraitHtml, getImageHtml, setImage, setImageFromGallery, setCropParams, getImageDataUrl, preloadAllImages } from '../store/ImageCache.js';
...
import {
    ...
    getCharPresetId, setCharPresetId
} from './roleData.js';
...
// ---- 渲染编辑表单 ----
// AI 预设下拉：不指定（走默认）+ 所有预设。绑的是预设 id（引用），改名/改配置都不会脱开；
// 预设被删时那条绑定会被一起清掉（roleData.unbindCharPresetsOf）⇒ 这里不必考虑"绑的那条不存在"。
// ★ 这里用 chara.id 而不是 chara.id || chara.base.name（本文件别处的惯例）：绑定挂在**角色 id** 上，
//   聊天与狼人杀都按真实 id 读取；无 id 的角色实际不存在（五类写入口全经 generateId），
//   用名字兜底只会在异常存档下写进一个永远读不到的键 —— 显示"已绑"却不生效，最难查。
function renderPresetSelect(chara) {
    const presets = getPresets();
    const bound = getCharPresetId(chara.id);
    const opts = presets.map(p =>
        // 给「当前全局默认」那条挂个括号备注（名字可以改，光看名字认不出哪条是默认）
        `<option value="${esc(p.id)}"${p.id === bound ? ' selected' : ''}>${esc(p.name)}${p.isDefault ? '（默认）' : ''}</option>`
    ).join('');
    return `
    <div style="margin-bottom:12px; background:#f5f5f5; border-radius:8px; padding:10px;">
        <label style="font-size:12px; color:#888; display:block; margin-bottom:4px;">🔌 AI 预设</label>
        <select id="editPreset" style="width:100%; border:1px solid #ccc; border-radius:6px; padding:8px; font-size:14px; box-sizing:border-box;">
            <option value=""${bound ? '' : ' selected'}>不指定（用默认预设）</option>
            ${opts}
        </select>
        <div style="font-size:11px; color:#999; margin-top:4px; line-height:1.5;">
            只在「这一个角色单独说话」时生效：主聊天回复、狼人杀里只扮演 TA 一个人的那些拍。
            多人同场（狼队里 AI 要扮好几只狼）仍走默认预设。
        </div>
    </div>`;
}
...
// ---- 渲染新建角色页（全屏子页，取代原来的 300px 模态框）----
// 字段 id 与旧模态框完全一致，AI 填充与提交逻辑据此可直接复用
function renderCreateForm() {
    return `
        <div class="screen-page">
            <div class="screen-header rb-header">
                <div class="screen-title">📝 新建角色</div>
            </div>
            <div class="screen-content">
                <div class="rb-page">

                    <div class="rb-ai">
                        <div class="rb-ai__title">✨ 让 AI 起个头</div>
                        <div class="rb-ai__hint">一句话描述想要的角色，下面的空白字段它会帮你填满，填完还能改。</div>
                        <textarea id="aiDescInput" rows="3" placeholder="例如：一个嘴硬心软的旧书店老板，欠着一笔人情债……"></textarea>
                        <button class="rb-btn rb-btn--ai" id="aiGenerateBtn">✨ 生成并填入</button>
                        <div class="rb-status" id="aiDescStatus"></div>
                    </div>

                    <div class="rb-or">或 自 己 填</div>

                    <div class="rb-sec">
                        <div class="rb-sec__title">基础</div>
                        <div class="rb-card">
                            <label class="rb-field">
                                <span>名称 <em class="rb-req">*</em></span>
                                <input id="newCharName" type="text" placeholder="必填" />
                            </label>
                            <div class="rb-row">
                                <label class="rb-field">
                                    <span>性别</span>
                                    <input id="newCharGender" type="text" placeholder="男 / 女 / 未知" />
                                </label>
                                <label class="rb-field">
                                    <span>年龄</span>
                                    <input id="newCharAge" type="text" placeholder="24岁 / 少年" />
                                </label>
                            </div>
                            <label class="rb-field">
                                <span>性取向</span>
                                <input id="newCharOrientation" type="text" placeholder="异性恋 / 同性恋 / 双性恋 / 未知" />
                            </label>
                        </div>
                    </div>

                    <div class="rb-sec">
                        <div class="rb-sec__title">性格与设定</div>
                        <div class="rb-card">
                            <label class="rb-field">
                                <span>描述</span>
                                <textarea id="newCharDesc" rows="2" placeholder="角色的基本情况……"></textarea>
                            </label>
                            <label class="rb-field">
                                <span>说话风格</span>
                                <textarea id="newCharStyle" rows="2" placeholder="说话方式、语气特点……"></textarea>
                            </label>
                            <label class="rb-field">
                                <span>详细设定</span>
                                <textarea id="newCharDetail" rows="4" placeholder="外貌特征、性格特点、背景故事……"></textarea>
                            </label>
                            <label class="rb-field">
                                <span>内心秘密</span>
                                <textarea id="newCharSecret" rows="2" placeholder="这个角色不为人知的秘密……"></textarea>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="rb-sec">
                        <details class="rb-fold">
                            <summary>📖 公开信息（分层，可选）</summary>
                            <div class="rb-fold__body">
                                <div class="rb-hint">好友 / 挚友能看到的额外信息，留空则只有公开名片。</div>
                                <label class="rb-field">
                                    <span class="rb-lv">L0 表象层</span>
                                    <textarea id="newCharProfileL0" rows="2" placeholder="第一眼看到的表象：表面性别、长相、气质……"></textarea>
                                </label>
                                <label class="rb-field">
                                    <span class="rb-lv">L1 熟人层</span>
                                    <textarea id="newCharProfileL1" rows="2" placeholder="好友能看到的性格细节……"></textarea>
                                </label>
                                <label class="rb-field">
                                    <span class="rb-lv">L2 密友层</span>
                                    <textarea id="newCharProfileL2" rows="2" placeholder="挚友才知道的秘密、弱点……"></textarea>
                                </label>
                                <label class="rb-field">
                                    <span class="rb-lv">L3 专属层</span>
                                    <textarea id="newCharProfileL3" rows="2" placeholder="每行一条 → 角色ID：内容&#10;char_主角：只有TA知道的秘密"></textarea>
                                </label>
                                <label class="rb-field">
                                    <span class="rb-lv">手动指定</span>
                                    <textarea id="newCharProfileManual" rows="2" placeholder="每行一条 → 角色ID：0/1/2&#10;char_法师：2"></textarea>
                                </label>
                            </div>
                        </details>
                    </div>
                    ...
                    <div class="rb-sec">
                        <div class="rb-sec__title">创建后</div>
                        <label class="rb-check">
                            <input type="checkbox" id="newCharAutoFriend" checked />
                            <span>自动加为当前主视角角色的好友</span>
                        </label>
                    </div>

                    <div class="rb-status" id="createCharStatus"></div>

                    <button class="rb-btn rb-btn--primary rb-btn--block" id="confirmNewChar">创建</button>
...
// ---- 新建角色页：事件绑定 ----
function bindCreateFormEvents(container, { memoryService, globalState }) {
    const rerender = () => {
        isCreating = false;
        const appContainer = container.closest('.screen-page') || container;
        appContainer.innerHTML = render({ memoryService, globalState });
    ...
    // ★ AI 辅助生成：读「让 AI 起个头」的描述 + 已填字段拼上下文，只填补空白项
    container.querySelector('#aiGenerateBtn')?.addEventListener('click', async () => {
        const desc = container.querySelector('#aiDescInput')?.value?.trim();
        const statusEl = container.querySelector('#aiDescStatus');
        if (!desc) {
            flash(statusEl, '⚠️ 请先在描述框中填写角色描述');
            return;
        }
        ...
        const existing = {
            ...
            gender: document.getElementById('newCharGender')?.value.trim(),
            age: document.getElementById('newCharAge')?.value.trim(),
            orientation: document.getElementById('newCharOrientation')?.value.trim(),
        ...
        const btn = container.querySelector('#aiGenerateBtn');
        ...
        try {
            ...
            fill('newCharName', result.name);
            fill('newCharGender', result.gender || '未知');
            fill('newCharAge', result.age || '未知');
            fill('newCharOrientation', result.orientation || '未知');
            fill('newCharDesc', result.desc);
            fill('newCharStyle', result.style);
            fill('newCharSecret', result.secret);
            fill('newCharDetail', result.detail);

            // ★ 公开信息只回填 L0/L1/L2（L3 / 手动指定按角色 ID 指定，AI 无从得知）
            if (result.profile) {
                const p = result.profile;
                if (typeof p.L0 === 'string') fill('newCharProfileL0', p.L0);
                if (typeof p.L1 === 'string') fill('newCharProfileL1', p.L1);
                if (typeof p.L2 === 'string') fill('newCharProfileL2', p.L2);
            }
    ...
    // ★ 确认创建
    container.querySelector('#confirmNewChar')?.addEventListener('click', () => {
        const name = container.querySelector('#newCharName')?.value.trim();
        if (!name) {
            flash(container.querySelector('#createCharStatus'), '⚠️ 请输入角色名称');
            return;
        }

        const newChar = createCharacterByName(name, {
            gender: document.getElementById('newCharGender')?.value.trim() || '未知',
            age: document.getElementById('newCharAge')?.value.trim() || '未知',
            orientation: document.getElementById('newCharOrientation')?.value.trim() || '未知',
            desc: document.getElementById('newCharDesc')?.value.trim() || '',
            style: document.getElementById('newCharStyle')?.value.trim() || '',
            secret: document.getElementById('newCharSecret')?.value.trim() || '',
        ...
        // ★ 根据复选框决定是否自动加好友
        if (container.querySelector('#newCharAutoFriend')?.checked) {
            autoAddFriend(newChar, globalState?.activeCharacter);
        }
...
export function bindEvents(container, { memoryService, globalState }) {
    ...
    if (isCreating) {
        bindCreateFormEvents(container, { memoryService, globalState });
        return;
    }
    ...
    if (viewingIndex !== null) {
        ...
        if (saveBtn) {
            saveBtn.addEventListener('click', () => {
                ...
                // ★ 角色绑的 AI 预设（名册侧的 ai_char_presets 表，见 roleData.js）；选「不指定」= 解绑
                setCharPresetId(chara.id, document.getElementById('editPreset')?.value || '');
                ...
                choiceOverlay.querySelector('.choice-album').onclick = () => {
                    ...
                    import('./gallery.js').then(gallery => {
                        gallery.renderGalleryPicker(async (galleryKey) => {
                            ...
                            // ★ ratio: 1 —— 形象卡容器是 80×80 的方块，必须锁正方形，
                            //   否则用户框个长条出来就被塞进方框里拉伸变形。
                            const crop = await showCropEditor(dataUrl || galleryKey, { ratio: 1 });
                            // ★ 取消（crop === null）也要写：图已经换成新的了，
                            //   旧参数留着会套到新图上，位置全错。
        ...
        // ★ 新建角色：切到全屏子页（表单见 renderCreateForm）
        container.querySelector('#createCharBtn')?.addEventListener('click', () => {
