...
import { isActivated, mountLockScreen } from './store/activation.js';
...
function renderHome() {
    ...
    const bottomNavIds = ['settings', 'market', 'worldDictionary'];
...
function render() {
    // ★ 激活锁屏：未激活时整个应用停在锁屏，拦截一切路由渲染
    if (!isActivated()) {
        elements.statusBackBtn.style.visibility = 'hidden';
        elements.creatorOverlay.classList.remove('show');
        elements.bottomNav.classList.add('hidden');
        elements.pageContainer.style.overflow = 'hidden';
        document.body.classList.add('locked');
        // lockBound 防止锁定时反复触发导航导致锁屏重挂、丢失输入焦点
        if (elements.pageContainer.dataset.lockBound !== 'true') {
            elements.pageContainer.dataset.lockBound = 'true';
            mountLockScreen(elements.pageContainer, {
                onSuccess() {
                    document.body.classList.remove('locked');
                    state.stack = ['home'];   // 解锁后固定回到首页
                    delete elements.pageContainer.dataset.lockBound;
                    render();
                }
            });
        }
    ...
    // ★ 读取全局设置，控制状态栏显示（缺省 = 不显示，键为 'true' 才显示）
...
async function init() {
    ...
    try {
        initDesktopInteraction({ globalState });
    } catch (error) {
    ...
    // init() 函数中（缺省 = 不显示手机壳，键为 'true' 才显示）
