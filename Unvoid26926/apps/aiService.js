...
// 边界：预设本体（ai_presets：地址/密钥/模型/上下文上限）住这一层；**「角色 → 预设」的绑定不在这一层**，
// 它属于角色数据侧（apps/roleData.js 的 ai_char_presets）——这一层只认调用方传进来的 presetId，不认识角色。
...
export async function callAI({ systemPrompt, assistantContext, userMessage, presetId, maxTokens } = {}) {
    const preset = presetId ? getPreset(presetId) : getDefaultPreset();
    ...
    const messages = [
        ...
        { role: 'user', content: userMessage || '请继续对话' }
    ];

    return _callAI(preset, messages, {
        maxTokens: maxTokens || 3000,  // 如果传了就覆盖默认值
        temperature: 0.8
    });
...
async function _callAI(preset, messages, { maxTokens = 3000, temperature = 0.8 } = {}) {
    ...
    // console.log('📥 API 完整响应:', data);
    // ★ 打印 API 返回结构（精简版）
    console.log('📥 API 返回结构:', {
        model: data.model,
        id: data.id,
        usage: data.usage,
        choices: data.choices?.map(c => ({
            finish_reason: c.finish_reason,
            content_preview: c.message?.content?.slice(0, 80) + '...',
            reasoning: c.message?.reasoning_content ? '✅ 有思考链' : '❌ 无思考链',
            tool_calls: c.message?.tool_calls?.length || 0
        }))
    });
    return {
 ...
 */
export async function callAIForMemoryExtract({ systemPrompt, assistantContext, presetId, userMessage } = {}) {
    const preset = presetId ? getPreset(presetId) : getDefaultPreset();
    if (!preset) throw new Error('未找到可用的 API 预设');
    if (!preset.apiKey) throw new Error('请先在设置中填写 API 密钥');
    ...
    const messages = [
        ...
        { role: 'user', content: userMessage || '请根据以上对话文本输出符合格式的内容' }
    ];

    const result = await _callAI(preset, messages, { maxTokens: 4096, temperature: 0.6 });
...
export async function callAIForPixel({ description, presetId, systemPrompt, taskInfo } = {}) {
    ...
    const doCall = () => Promise.race([
        _callAI(preset, messages, { maxTokens: 3000, temperature: 0.2 }).then(r => r.content),
        timeoutPromise
    ]);
...
export async function callAIForStory({ systemPrompt, description, presetId } = {}) {
    ...
    const result = await Promise.race([
        _callAI(preset, messages, { maxTokens: 8000, temperature: 0.7 }),
        timeoutPromise
    ]);
...
// ============================================================
//  通用 AI 调用接口（供 txtImporter 等工具模块使用）
// ============================================================

/**
 * 通用 AI 调用：传入 systemPrompt + userContent，返回 AI 回复
 * @param {object} options
 * @param {string} options.systemPrompt - 系统提示词
 * @param {string} options.userContent - 用户消息
 * @param {number} [options.maxTokens=4096] - 最大 Token 数
 * @param {number} [options.temperature=0.7] - 温度
 * @param {string} [options.presetId] - 指定预设 ID（不传则用默认）
 * @returns {Promise<string>} AI 回复文本
 */
export async function callAIWithMessages({ systemPrompt, userContent, maxTokens = 4096, temperature = 0.7, presetId } = {}) {
    const preset = presetId ? getPreset(presetId) : getDefaultPreset();
    if (!preset) throw new Error('未找到可用的 API 预设');
    if (!preset.apiKey) throw new Error('请先在设置中填写 API 密钥');
    ...
    const result = await _callAI(preset, messages, { maxTokens, temperature });
    return result.content;
}

/**
 * 通用多轮调用：**直接吃一个拼好的 messages 数组**，不改它的形状。
 *
 * 上面那几个 `callAIXxx` 都是「两段式」包装（各自写死 system + 一条 user），
 * 多轮的调用方（文游：一轮一对 user/assistant）在这里没有位置，所以单开这个出口。
 * 它不认业务、不拼内容、只负责取预设 + 校验 + 转发 —— 形状由调用方负责。
 *
 * @param {object} options
 * @param {Array<{role:string, content:string}>} options.messages - 已经拼好的完整消息数组
 * @param {number} [options.maxTokens=4096]
 * @param {number} [options.temperature=0.7]
 * @param {string} [options.presetId] - 不传则用默认预设
 * @returns {Promise<string>} AI 回复文本
 */
export async function callAIConversation({ messages, maxTokens = 4096, temperature = 0.7, presetId } = {}) {
    const preset = presetId ? getPreset(presetId) : getDefaultPreset();
    if (!preset) throw new Error('未找到可用的 API 预设');
    if (!preset.apiKey) throw new Error('请先在设置中填写 API 密钥');
    if (!Array.isArray(messages) || messages.length === 0) throw new Error('没有要发送的消息');
