...
import { isArchived, getCharPresetId } from './roleData.js';
import { getSignature, applySignatureUpdate } from './signature.js';
import { collectMoments, formatTime } from './chat/momentsData.js';   // ★ 朋友圈取数走数据层（与朋友圈页同口径）

// ---- 自动记忆开关 ----
// 语义：查询「某个角色」的自动记忆开关是否开启
// 键 auto_memory_<角色ID> 不存在时视为开启（默认开），除非手动关闭
function isAutoMemoryOn(characterId) {
    return localStorage.getItem('auto_memory_' + characterId) !== 'false';
}
...
function friendCardBlockHtml(id, name, senderId) {
    return `<div class="chat-friend-card" data-friend-id="${esc(id)}" data-sender-id="${esc(senderId)}">
...
export async function injectChatMessage(activeId, otherId, message, options = {}) {
    ...
    const { systemPrompt, assistantContext, userMessage } = buildPrompt({
        ...
        autoMemory: isAutoMemoryOn(otherId)
    });
    ...
    try {
        const aiResult = await callAI({
            ...
            assistantContext, userMessage, maxTokens: options.maxTokens || 4096,
            presetId: getCharPresetId(otherId)      // 说话的是 otherId：他绑过预设就用他的，没绑 = '' ⇒ 走默认
        });
        reply = (aiResult?.content || aiResult || '').trim();
    } catch (e) { reply = `⚠️ ${e.message}`; }
    const displayReply = reply.replace(/【(记忆|修改记忆|删除记忆|关系|态度|认知|个签)】.+?(?=\n|$)/g, '').replace(/\n{2,}/g, '\n').trim();
    ...
    return `
        <div class="contacts-section">
            ...
            <div class="contacts-list">
                ...
                ${contacts.map((item) => {
                    // ★ 个性签名优先，无签名回退 desc
                    const sig = getSignature(item.id);
                    return `
    ...
    <div class="contact-avatar-wrapper" data-contact-id="${item.id}" style="width:36px; height:36px; border-radius:50%; overflow:hidden; flex-shrink:0; background:#e0e0e0; cursor:pointer; position:relative;">
        ...
        <div style="position:absolute; bottom:0; left:0; right:0; background:rgba(0,0,0,0.4); color:white; font-size:10px; text-align:center; padding:2px 0; opacity:0; transition:opacity 0.2s;">更换</div>
                        ...
                        <div style="flex:1; min-width:0;">
                            ...
                            <div class="contact-note">${esc(sig || item.note) || ''}</div>
                        </div>
                    </div>
                `;
                }).join('')}
...
function renderDiscoverPage(globalState) {
    const activeId = getActiveCharacterId(globalState);
    const recent = collectMoments(activeId).slice(0, 3);   // ★ 可见动态（主视角 + 好友）最新 3 条
    return `
        <div class="discover-section">
            <div class="discover-card">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px;">
                    <div class="discover-title" style="margin-bottom:0;">朋友圈</div>
                    <button id="momentsEntryBtn" class="discover-arrow-btn" aria-label="进入朋友圈">
                ...
                ${recent.length === 0
                    ? '<div style="color:#999;font-size:13px;padding:8px 0;">还没有动态</div>'
                    : recent.map(m => `
                    <div class="moment-item">
                        <div class="moment-title">${esc(m.authorId === activeId ? '我' : (getCharacterNameById(m.authorId) || '?'))}</div>
                        <div class="moment-subtitle">发布了动态</div>
...
function renderMePage(globalState) {
    ...
    const signature = getSignature(chara.id);
    return `
        <div class="me-section">
            <div class="profile-card">
                <div id="avatarUploadContainer" style="width:60px; height:60px; border-radius:50%; overflow:hidden; background:#e0e0e0; cursor:pointer;">
                    ${getAvatarHtml(chara.id)}
                </div>
                <div class="profile-info">
                    <div class="profile-name">${esc(chara.base.name)}</div>
                    ${signature ? `<div class="profile-signature">${esc(signature)}</div>` : ''}
                    <div class="profile-desc">${esc(chara.base.desc)}</div>
                </div>
            </div>
        ...
        `).join('');
            ...
            <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 16px;background:white;border-radius:12px;margin-top:12px;">
                <div>
                    ...
                    <div style="font-size:12px;color:#999;">每次对话满10轮时自动总结记忆</div>
                </div>
        ...
        <div class="chat-detail">
            <div class="chat-detail-header">
                ...
                <button id="autoMemoryToggle" class="btn-sm" style="
                    background:${isAutoMemoryOn(otherId) ? '#4CAF50' : '#ccc'};
                    margin-right:8px;
                ">${isAutoMemoryOn(otherId) ? '🧠' : '🧠 off'}</button>
...
${messages.map((msg, msgIndex) => {
        if (msg?.type === 'lingxi') {
            const ref = msg.lingxiRef || {};

            return `
                <div
                    class="lingxi-system-message"
                    data-topic-id="${esc(ref.topicId || '')}"
                    data-lingxi-pair-key="${esc(ref.lingxiPairKey || '')}"
                >
                    <div class="lingxi-system-title">
                        🔗 灵犀话题
                    </div>
        ...
        const bubbleHtml = renderMsgContent(
            msg.text,
            msg.senderId,
            isMe,
            pairKey
...
<div class="chat-input-area">
   ...
   autocomplete="one-time-code" inputmode="text" name="chat_msg_${pairKey}">
    ...
    <!-- display 必须留在内联：JS 用 style.display='' 显示它，挪进 CSS 后置空会回落到 none -->
    <button id="chatSendAiBtn" style="display:none; flex-shrink:0;" title="请求 AI 回复">⚡</button>
</div>
        </div>
    `;
}
...
export function render({ globalState } = {}) {
    const pageContent = activePage === 'chats'
        ...
        : activePage === 'contacts'
            ...
            : activePage === 'discover'
                ? renderDiscoverPage(globalState)
    ...
    if (activePage === 'contacts') {
        ...
        container.querySelectorAll('.start-chat-btn').forEach(btn => {
            btn.addEventListener('click', function () {
                openChatWith(this.dataset.contactId);
            });
        });

        // ★ 通讯录整条可点：去掉了「💬 聊天」按钮，点卡片任意处都开聊。
        //   头像那块自己有点击（换头像），得在开头拦掉，否则点一次头像会连聊天一起开了。
        container.querySelectorAll('.contact-card[data-contact-id]').forEach(card => {
            card.addEventListener('click', function (e) {
                if (e.target.closest('.contact-avatar-wrapper')) return;
                openChatWith(this.dataset.contactId);
            });
        });
...
function bindChatDetailEvents(container, pairKey, otherId, memoryService, globalState) {
    ...
    chatMessages?.addEventListener('click', (e) => {
        const card = e.target.closest('.chat-friend-card');
        if (!card) return;
...
`;
    ...
    async function callAIAndRenderReply() {
        ...
        // ★ 显示"对方正在输入……"
        // 结构与真实消息完全一致（.msg-row.other + .msg-avatar + .msg-bubble），
        // 尺寸/位置随之一致，避免出现与消失时的跳动
        const typingMsg = document.createElement('div');
        typingMsg.className = 'msg-row other';
        typingMsg.id = 'typingIndicator';
        typingMsg.innerHTML = `
    <div class="msg-avatar">${getAvatarHtml(otherId)}</div>
    <div style="min-width:0;">
        <div class="msg-bubble other chat-typing">✏️ 正在输入……</div>
    </div>
`;
        ...
        setTimeout(async () => {
            ...
            try {
                ...
                // ★ 名册里没有 → 查世界角色网络（与 416-420 那条发送路径一致）
                //   不查这一跳的话，人设只剩 char_<id>.info 那几个字段（name/emoji/desc/type/label），
                //   说话风格、内心秘密、详细设定都会缺
                if (!characterData) {
                    characterData = JSON.parse(localStorage.getItem('worldnet_extra_characters') || '[]').find(c => c.id === otherId) || null;
                }
                // ★ 两个列表都没有，才从 CharacterStore 兜底造一份
                //   id 必须带上：下面写关系/态度用的是 new CharacterStore(characterData.id)，
                //   漏掉就变成 char_undefined——世界网络角色的关系会全挤进同一个桶
                if (!characterData) {
                    const store = new CharacterStore(otherId);
                    const info = store.getInfo();
                    if (info.name) {
                        characterData = { id: otherId, base: info };
                    }
                }
            } catch (e) { }

            const allMessages = getOrCreateMessages(pairKey);
            const promptMessages = await expandLingxiMessages(
            ...
            try {
                const aiResult = await callAI({
                    ...
                    userMessage,
                    maxTokens: 4096,
                    presetId: getCharPresetId(otherId)      // 同上：这一条回复是 otherId 说的，就用他绑的预设
                });
            ...
            // ★ 自动记忆处理
            if (isAutoMemoryOn(otherId)) {
                const store = new CharacterStore(otherId);
                // 标记不要求独占一行（2026-09-18）：提示词只说「在回复末尾加 【记忆】…」，
                // 还要求「用 | 分隔短句」，模型常把标记跟在句尾——逐行 ^【记忆】 锚行首会整条丢掉。
                // 内容取到行尾或下一个【标记】为止；箭头全角/半角都认。
                for (const m of reply.matchAll(/【记忆】([^\n【]*)/g)) {
                    const content = m[1].trim().replace(/^[：:]\s*/, '');   // 模型偶尔写「【记忆】：xxx」
                    if (!content) continue;
                    store.addMemory({
                ...
                for (const m of reply.matchAll(/【修改记忆】([^\n【]*?)\s*(?:→|->)\s*([^\n【]+)/g)) {
                    const quoted = m[1].trim();
                    const memories = store.getMemories();
                    const idx = memories.findIndex(em => em.content === quoted);
                    if (idx >= 0) {
                        memories[idx].content = m[2].trim();
                        memories[idx].time = new Date().toLocaleString('zh-CN');
                        store._save();
                    } else {
                        console.warn('【修改记忆】没找到对应原文，已跳过：', quoted);
                    }
                }
                // ★ 关系网解析
                ...
                // ★ 个性签名（挂在自动记忆开关下，与记忆/关系/态度/认知同一总闸）
                // 提示词侧同样只在 autoMemory 开时注入签名段 → 关掉开关 = 角色不做任何自主维护
                const sigMatch = reply.match(/【个签】(.+)/);
                if (sigMatch) {
                    reply = reply.replace(/【个签】.+/, '').trim();
                    applySignatureUpdate(otherId, sigMatch[1].trim(), { source: 'chat' });
                }
            }
            ...
            // ★ 清理回复文本（去掉记忆操作行；非贪婪 + \n 前瞻：标记不在句末也能整行剥除）
            const displayReply = reply.replace(/【(记忆|修改记忆|删除记忆|个签)】.+?(?=\n|$)/g, '').replace(/\n{2,}/g, '\n').trim();

            // ④ 移除"正在输入……"
            const typingEl = chatMessages.querySelector('#typingIndicator');
...
`;
    ...
    container.querySelector('#autoMemoryToggle')?.addEventListener('click', () => {
        ...
        const current = isAutoMemoryOn(otherId);
        localStorage.setItem(key, current ? 'false' : 'true');
        // 重渲染聊天界面
        const appContainer = container.closest('.screen-page') || container;
        appContainer.innerHTML = render({ globalState });
    ...
    // ★ 灵犀消息折叠/展开
    chatMessages?.addEventListener('click', (e) => {
        const toggle = e.target.closest('.lingxi-system-toggle');
        if (!toggle) return;

        const detail = toggle.nextElementSibling;
        if (!detail || !detail.classList.contains('lingxi-system-detail')) return;

        const isOpen = detail.classList.toggle('is-open');
        toggle.setAttribute('aria-expanded', isOpen);

        const arrow = toggle.querySelector('.lingxi-system-arrow');
        if (arrow) {
            arrow.textContent = isOpen ? '⌃' : '⌄';
        }
    });

    function insertLingxiMessageHistory(topicId, lingxiPairKey) {
        if (!topicId || !lingxiPairKey) return false;

        const messages = getOrCreateMessages(pairKey);

        const exists = messages.some(message =>
            message?.type === 'lingxi' &&
            message?.lingxiRef?.topicId === topicId &&
            message?.lingxiRef?.lingxiPairKey === lingxiPairKey
        );

        if (exists) {
            showToast('这个灵犀话题已经在当前对话中');
            return false;
        }
    ...
    try {
        ...
        if (chatDetail) {
            initChatUI(chatDetail, {
                globalState,
                otherId,
                activeId,
                contact,
                pairKey,
                getCharacterName: getCharacterNameById,

                // ChatUI 选择并展示后，只通知 chat.js 保存最小历史引用。
                onLingxiTopicSelected: ({ topicId, lingxiPairKey }) =>
                    insertLingxiMessageHistory(topicId, lingxiPairKey)
            });
        }
    } catch (e) {
...
async function expandLingxiMessages(messages, viewerId) {
    const {
        getPairTopic,
        projectPairTopic
    ...
    for (const message of messages) {
        if (message?.type !== 'lingxi') {
            result.push(message);
            continue;
        }

        const ref = message.lingxiRef || {};
        ...
        const responseText = (projection.responses || [])
            .map(response =>
                `【${response.label}】\n${response.text}`
            )
            .join('\n\n');
        ...
        result.push({
            ...
            senderDisplayName: '灵犀',
            text: [
                '这是聊天历史中的灵犀系统消息，不是任何角色发送的普通消息。',
                projection.isOwnPost
                    ? '这是你之前发布的灵犀帖子。'
                    : '这是灵犀中出现过的一条匿名帖子。',
                projection.topicName
                    ? `话题：${projection.topicName}`
                    : '',
                `帖子：${projection.topicText || ''}`,
                responseText,
                '可以作为对话历史的补充信息。',
                '不用过于在意。'
            ].filter(Boolean).join('\n\n')
...
async function extractMemoriesForActiveChar(activeId, otherId, otherName, allMessages) {
    try {
        ...
        // 记忆提取属于当前主视角角色，因此使用 activeId 展开灵犀内容。
        // activeId 同时也是后面保存记忆的角色 ID。
        const memoryMessages = await expandLingxiMessages(
            recentMessages,
            activeId
        );

        const convText = memoryMessages.map(message => {
            // 灵犀已经由 expandLingxiMessages() 按 activeId 展开，
            // 这里直接保留它生成的角色视角文本。
            if (
                message?.type === 'system' &&
                message?.senderDisplayName === '灵犀'
            ) {
                return [
                    '【灵犀系统消息】',
                    message.text || '灵犀话题已无法读取。'
                ].join('\n');
            }

            const displayName = message.senderId === activeId
                ? '我'
                : (
                    getCharacterNameById(message.senderId) ||
            ...
            return `${displayName}：${String(message.text || '')
                .replace(/\|/g, '')}`;
        }).join('\n');
