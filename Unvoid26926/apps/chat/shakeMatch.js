...
async function generateIcebreakerTasks(cfg, who) {
    ...
    console.warn('摇一摇破冰任务生成失败，退回内置任务。原始返回：', raw);
    return {
...
// ★ 任务完成判定：pool = 指定方说过的消息、otherPool = 另一侧说过的（都按 side 取，见 saidBy）
function checkRuleDone(rule, pool, otherPool) {
    if (!rule) return false;
    if (rule.rule === 'both') {
        const n = rule.minEach || 1;
        return pool.length >= n && (otherPool || []).length >= n;
    }
    // ★ emoji-only：指定方最近连续 min 条消息只含 emoji
    if (rule.rule === 'emoji') {
        const n = rule.min || 3;
        const last = pool.slice(-n);
        return last.length >= n && last.every(m => /^[\p{Emoji_Presentation}\p{Emoji}\s]+$/u.test((m.text || '').trim()));
    }
...
// ★ 主视角坐在哪一侧
function viewerSide(cv, activeId) {
    return activeId === cv.initiatorId ? 'initiator' : 'receiver';
}

// ★ 这条消息是哪一侧说的：'initiator' | 'receiver'。新消息认 side——它是绝对的
//   （谁发的跟当前是谁在看无关），所以切主视角不会过期。早于 side 的消息只能按 from
//   近似：from 是「当时」的相对标签（AI 那侧一律字面量 'mystery'＝「当时的对面」），
//   切过视角的那些会归错边。陌生人池那一侧没有角色 id，也只能走 side。
function sideOf(m, cv, activeId) {
    if (m.side) return m.side;
    const vs = viewerSide(cv, activeId);
    return m.from === activeId ? vs : (vs === 'initiator' ? 'receiver' : 'initiator');
}

// ★ 这条消息是不是「我」说的
function isMine(m, cv, activeId) {
    return sideOf(m, cv, activeId) === viewerSide(cv, activeId);
}

// ★ 某一侧说过的消息（系统提示不算谁的发言）
function saidBy(cv, activeId, side) {
    return (cv.messages || []).filter(m => m.from !== 'system' && sideOf(m, cv, activeId) === side);
}

// ★ 检查双方任务：initiatorTask 由发起者引导、完成看接收者发言；receiverTask 反之。提示按当前视角
async function checkTasks(cv, activeId) {
    if (!cv.task) return false;
    const mySide = viewerSide(cv, activeId);
    let changed = false;
    for (const key of ['initiatorTask', 'receiverTask']) {
        const t = cv.task[key];
        if (!t || t.done || t.failed) continue;
        // 这个任务由哪一侧引导（＝持有方，自己不能说出 target）；完成要看的是另一侧
        const holderSide = key === 'initiatorTask' ? 'initiator' : 'receiver';
        const watchSide = holderSide === 'initiator' ? 'receiver' : 'initiator';
        const mine = holderSide === mySide;
        ...
        if (t.target) {
            const selfSaid = saidBy(cv, activeId, holderSide).some(m => (m.text || '').includes(t.target));
            if (selfSaid) {
                t.failed = true;
                cv.messages.push({ from: 'system', text: `❌ ${mine ? '你的' : '对方的'}任务失败：「${t.title}」`, time: Date.now() });
                changed = true;
                continue;
            }
        }
        // ★ 完成：指定方说出目标词 / 满足规则
        if (checkRuleDone(t.rule, saidBy(cv, activeId, watchSide), saidBy(cv, activeId, holderSide))) {
            t.done = true;
            cv.messages.push({ from: 'system', text: `✅ ${mine ? '你' : '对方'}完成了任务：「${t.title}」`, time: Date.now() });
            changed = true;
        }
    }
...
async function renderChat(overlay, globalState, onBack, cv) {
    ...
    const send = async () => {
        ...
        cv2.messages.push({ from: activeId, side: activeId === cv2.initiatorId ? 'initiator' : 'receiver', text, time: Date.now() });
        ...
        try {
            await taskManager.watch('shake', `摇一摇回复 · ${chatTitle(cv2)}`, async () => {
                const { callAIConversation } = await import('../aiService.js');
                ...
                const systemPrompt = '下面是你和对方的聊天记录，接着对方最后那句往下说。' +
                    '你现在是一个真实的人，深夜的陌生社交里，和一个陌生人聊上了。' +
                    '你叫「' + (who.name || '不愿说名字的人') + '」' +
                ...
                // ★ 历史按 user/assistant 交替送，不再摊平成一段「我：/神秘人：」文本。
                //   摇一摇里对面没有身份要承载——AI 那一侧就是 assistant，当前主视角那一侧就是 user。
                //   两人不一定你一句我一句，所以连续同一侧合并成一条。不插占位消息：
                //   占位是一句能被回应的话，模型会去接它。
                const turns = [];
                for (const m of (cv2.messages || []).filter(m => m.from !== 'system').slice(-8)) {
                    const role = isMine(m, cv2, activeId) ? 'user' : 'assistant';
                    const last = turns[turns.length - 1];
                    if (last && last.role === role) last.content += '\n' + (m.text || '');
                    else turns.push({ role, content: m.text || '' });
                }
                // 收尾是主视角刚发的那句；开头得是 user——切过视角后可能剩下 assistant
                while (turns.length && turns[0].role !== 'user') turns.shift();
                const raw = await callAIConversation({
                ...
                cv3.messages.push({ from: 'mystery', side: iAmInitiator ? 'receiver' : 'initiator', text: cleanText || '……', time: Date.now() });
