...
// ---- 相册数据管理 ----
// 返回全部相册，被隐藏的也在——隐藏记在条目自己的 `hidden` 上，不把条目剔出去。
// 剔出去会让「看不见」和「不存在」混成一件事，写入路径拿到的就不是全量了。
function getAlbums() {
    let merged;
    try {
    ...
    } catch { merged = [...DEFAULT_ALBUMS]; }

    // ★ 桌宠相册可被隐藏（设置项「在相册中显示桌宠精灵图上传入口」）——
    //   隐藏 ≠ 删除，条目连同 images 一直都在，只是显示端不列它。
    const showPetAlbum = localStorage.getItem('global_show_pet_album') === 'true';
    return merged.map(a => ({ ...a, hidden: a.type === 'pet' && !showPetAlbum }));
}
...
export function render() {
    const albums = getAlbums();
    const shown = albums.filter(a => !a.hidden);   // ★ 隐藏的相册不进列表，也进不去

    // 预置相册子页面
    if (viewingAlbum === 'album_avatar' || viewingAlbum === 'album_portrait' || viewingAlbum === 'album_pixel') {
        const album = shown.find(a => a.id === viewingAlbum);
...
function renderAlbumView(album) {
    const albums = getAlbums();
    const otherAlbums = albums.filter(a => !a.hidden && a.id !== album.id);
...
function renderMoveDialog(currentAlbumId) {
    const albums = getAlbums().filter(a => !a.hidden && a.id !== currentAlbumId);
...
export function renderGalleryPicker(onSelect) {
    ...
    albums.forEach(album => {
        ...
        if (album.hidden) return;               // 跳过被隐藏的相册
