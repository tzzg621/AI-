...
import { CharacterStore, generateId, createDefaultCharacterData } from '../../store/CharacterStore.js';
...
"profile":{"L0": "第一眼看到的表象：表面性别、长相、整体气质（一段纯文本，可以刻意与真实不符）",
    "L1":"好友能看到的性格细节（一段纯文本）",
    "L2":"挚友才知道的秘密、弱点（一段纯文本）"
},
"relations":[{"name":"对方名字","relation":"关系描述","perspective":"看法"}]}]
        ...
        ">
            ...
            <div style="flex:1; min-width:0; pointer-events:none;">
                <div style="display:flex; align-items:center; gap:8px; margin-bottom:4px;">
                    <span style="font-weight:600; font-size:15px; color:#333;">${esc(item.name || '未命名角色')}</span>
                    ${genderAge ? `<span style="font-size:12px; color:#999;">${esc(genderAge)}</span>` : ''}
                </div>
...
function renderWorldCard(item, index) {
    ...
    return `
        ...
        ">
            ...
            <div style="flex:1; min-width:0; pointer-events:none;">
                <div style="font-weight:600; font-size:15px; color:#333; margin-bottom:4px;">${esc(item.title || '未命名条目')}</div>
                <div style="font-size:13px; color:#666; line-height:1.4; display:-webkit-box; -webkit-line-clamp:2; -webkit-box-orient:vertical; overflow:hidden;">
                    ${esc(text)}
                </div>
            </div>
...
function renderEditModal() {
    ...
    return `
        ...
        ">
            ...
            ? `<textarea class="txt-modal-field" data-field="${f}" style="width:100%;padding:10px;border:1.5px solid #e8e0f0;border-radius:10px;font-size:13px;font-family:inherit;line-height:1.5;resize:vertical;min-height:${f === 'detail' ? '120px' : '60px'};outline:none;box-sizing:border-box;">${esc(item[f] || '')}</textarea>`
            : `<input class="txt-modal-field" data-field="${f}" value="${esc(item[f] || '')}" style="width:100%;padding:10px;border:1.5px solid #e8e0f0;border-radius:10px;font-size:13px;font-family:inherit;outline:none;box-sizing:border-box;">`
        }
                    </div>
                `).join('')}
...
function saveCharactersToRolebook(jsonStr) {
    ...
    // ★ 第一步：过滤出真正要新增的角色，预先生成 ID
    const newChars = chars
        .filter(c => c.name && !existing.some(e => e.base.name === c.name))
        .map(c => ({
            ...c,
            _id: generateId()  // 预生成 ID
        }));

    // ★ 第二步：保存到角色名册，并写入关系数据（优先锁定同批次的 ID）
    newChars.forEach(c => {
        const charData = createDefaultCharacterData(c._id, {
            name: c.name,
            gender: c.gender || '未知',
            age: c.age || '未知',
        ...
        if (c.relations && c.relations.length > 0) {
            const store = new CharacterStore(c._id);
            const relations = c.relations.map(r => {
                // ★ 先从同批次找同名角色
                const sameBatch = newChars.find(x => x.name === r.name);
                // ★ 再从已存在的角色名册找
                const existingChar = !sameBatch ? existing.find(e => e.base.name === r.name) : null;
                const targetId = sameBatch?._id || existingChar?.id || '';
                // ★ 如果找到了目标角色，双向加好友
                if (targetId) {
                    store.addFriend(targetId);
                    const targetStore = new CharacterStore(targetId);
                    targetStore.addFriend(c._id);
                }
                return {
                    name: r.name,
                    id: targetId,
        ...
        if (c.profile) new CharacterStore(c._id).setProfile(c.profile);
...
function saveCharactersToWorldNet(jsonStr) {
    ...
    chars.forEach(c => {
        ...
        // ★ 创建 char_xxx（联系人列表需要这个 key 存在才能显示）
        const charStore = new CharacterStore(npcData.id);
        charStore.setInfo({
            name: npcData.base.name,
            emoji: npcData.base.emoji,
            desc: npcData.base.desc,
            type: 'special',
            label: '特殊角色'
        });
        // ★ 补：分层公开信息
        if (c.profile) charStore.setProfile(c.profile);

        // ★ 保存关系数据
        if (c.relations && c.relations.length > 0) {
            // 查找同批次和已存在的角色来锁定 ID
            const allExisting = [...existing, ...JSON.parse(localStorage.getItem(STORAGE_KEY_ROLEBOOK) || '[]')];
            const store = new CharacterStore(npcData.id);
            const relations = c.relations.map(r => {
                const matched = allExisting.find(x => x.base?.name === r.name);
                const targetId = matched?.id || '';
                // ★ 如果找到了目标角色，双向加好友
                if (targetId) {
                    store.addFriend(targetId);
                    const targetStore = new CharacterStore(targetId);
                    targetStore.addFriend(npcData.id);
                }
                return {
                    name: r.name,
                    id: matched?.id || '',
