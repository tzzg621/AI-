...
// ★ 指令部分 → system
function buildSystemPrompt(mode, aiRoleName, targetName) {   // ★ 加参数
    const template = SYSTEM_TEMPLATES[mode] || SYSTEM_TEMPLATES.dual;
    return template(aiRoleName, targetName);
}
...
// ---- 个性签名：当前值 + 当日次数备注（信息段，无条件注入）----
// 更新指令在下方自动记忆块内（【个性签名管理】），同关系网/认知的写法：
// 信息归信息；要求 AI 主动维护的指令归自动记忆开关（那边是整段写好的文字）。
// 备注规则（用户 2026-09-11）：当日没更新过 → 保持原样、不加任何东西；
// 当日更新过 → 附「今天第 N 次更新」（配合管理段的固定上限，AI 自己看得出还剩几次）。
// 不新增存储字段——次数是已知信息，此处只做拼接引用（用户原则：不必要的数据不重复存储）。
// 仅在角色卡角色（character 非 null）时注入；元联系人 character 恒为 null → 天然不注入
function buildSignaturePrompt(characterId) {
    if (!characterId) return null;
    let current = '', note = '';
    try {
        current = new CharacterStore(characterId).getSignature();
        if (current) {
            const n = getTodayUpdateCount(characterId);
            if (n > 0) note = `（今天第 ${n} 次更新）`;
        }
    } catch { }
    if (!current) return null;   // 没写过 → 【个性签名管理】会说明它是什么，此处不占上下文
    return `【你的个性签名】
你当前挂着的是：「${current}」${note}`;
}
...
// ★ 尾部 user 消息
// 两个参数由调用方保证有值（名字查不到时落到 id，不会为空）
function buildReplyNudge(aiRoleName, targetName) {
    return `请以"${aiRoleName}"的身份，回应"${targetName}"发来的最新消息。`;
}
...
export function buildPrompt({
    ...
    // 读的是**说话这个人**绑的那条（2026-09-15）：他把这条对话交给哪条 API，上下文上限就该跟着那条走；
    // 没绑过 = '' ⇒ 读默认预设，与以前完全一样。
    if (maxTotalChars === undefined) {
        try {
            const bound = getCharPresetId(characterId);
            const preset = bound ? getPreset(bound) : getDefaultPreset();
            if (preset && preset.maxContextChars) {
                maxTotalChars = preset.maxContextChars;
            }
        } catch { /* 忽略 */ }
        if (maxTotalChars === undefined) {
    ...
    let systemPrompt = buildSystemPrompt(mode, aiRoleName, targetName);   // ★ 传 targetName
    ...
    // ★ 个性签名当前值（信息，无条件注入；更新指令在自动记忆块内）
    if (character) {
        const sigPart = buildSignaturePrompt(characterId);
        if (sigPart) systemPrompt += '\n\n' + sigPart;
    }
...
【推荐好友名片】
如果你决定把自己的朋友推荐给对方（例如对方想认识新朋友，或你主动引荐）：
  → 在对话回复中插入【名片:好友名字】
要求：
- 只能推荐上面【你与其他角色的关系】中列出过的角色
- 名字必须与关系网中的名字完全一致，不要编造不存在的角色
- 一次只推荐一个，无需解释名片本身`;

    systemPrompt += `

【接受好友推荐】
当对方给你发送【名片:名字】时，先判断你与该角色的关系：
- 如果该角色已经出现在上面的【你与其他角色的关系】中（已经认识/已是朋友）→ 自然回应"已经认识/已经是朋友了"，不要插入【接受好友:名字】
- 如果是不认识的新角色且你想加 → 在回复中插入【接受好友:名字】（名字要与名片中的一致）
- 不想加 → 不要插入该标记，自然表达拒绝即可`;
...
注意：
- 通常情况下，每句控制在 10-30 字左右。但如果人设或情境需要，可以无视字数要求，惜字如金或长篇大论都行。
- 用 | 来分隔句子，这是格式要求
- 不要改变你的角色语气和说话风格`;
...
- 如果没有变化，不需要加任何内容`;
        ...
        // 【个性签名管理】：要求 AI 主动维护的指令，归自动记忆总闸（签名当前值那半在 buildSignaturePrompt）
        // 这里的上限 3 与 apps/signature.js 的 DAILY_LIMIT 同源，改那边记得同步这里
        systemPrompt += `

【个性签名管理】
个性签名是你名片上的一行字，别人能看到，是你自己的心情窗口。

当你觉得此刻的情绪、心境或处境值得记下来时：
  → 在回复末尾单独一行加 【个签】新的签名内容
如果没有变化，不需要加任何内容

注意：
- 不超过 20 字，像随手写的一句心里话，不是台词、不是书面语
- 个签每天最多更新 3 次，用完了当天就不能再改
- 不要频繁更换，它才有分量`;
...
- 修改时，请尽量完整匹配已有记忆的原文`;
    ...
    const visibleProfilePart = (() => {
        ...
        try {
            let text = formatProfilePrompt(targetId, characterId, targetName);
            if (!text) return null;
            // ★ 对方的个性签名：公开信息，独立成行
            // （将来做「关键词提及 / 特别关注才注入」的过滤时，只动这一处）
            const targetSig = getSignature(targetId);
            if (targetSig) text += `\n个性签名：${targetSig}`;
            return { priority: 8, text };
        } catch { return null; }
    })();
    ...
    return {
        systemPrompt,
        assistantContext: combinedText || null,
        userMessage: buildReplyNudge(aiRoleName, targetName)
    };
}
