export function renderGlobalSettings() {
    // ★ 缺省值：状态栏、手机壳、桌宠相册 = 不显示（键为 'true' 才算开）；AI 任务中心 = 显示
    const showStatusBar = localStorage.getItem('global_show_status_bar') === 'true';
    ...
    const showPhoneFrame = localStorage.getItem('global_show_phone_frame') === 'true';
        ...
        <div class="screen-page">
            ...
            <div class="screen-content">
                <div class="page-card">
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; padding:14px 4px; border-bottom:1px solid #f0f0f0;">
    <div>
        <div style="font-weight:600; font-size:15px;">📱 手机壳</div>
        <div style="font-size:12px; color:#999; margin-top:2px;">显示黑色圆角的手机边框</div>
    </div>
    <button id="togglePhoneFrameBtn"
            style="padding:6px 16px; border-radius:14px; border:none;
                   background:${showPhoneFrame ? '#0b93f6' : '#ccc'};
                   color:white; cursor:pointer; font-size:13px; font-weight:600;">
        ${showPhoneFrame ? '🟢 显示' : '⚪ 隐藏'}
    </button>
</div>
...
export function bindGlobalEvents(container, onBack) {
    ...
    container.querySelector('#toggleStatusBarBtn')?.addEventListener('click', () => {
        const current = localStorage.getItem('global_show_status_bar') === 'true';
    ...
    container.querySelector('#togglePhoneFrameBtn')?.addEventListener('click', () => {
        const current = localStorage.getItem('global_show_phone_frame') === 'true';
        localStorage.setItem('global_show_phone_frame', current ? 'false' : 'true');

        // ★ 只需切换 body 类名（current=true 表示之前显示 → 现在隐藏，加类）
        document.body.classList.toggle('no-frame', current);
