...
import {
    buildTurnPrompt, findOptionBlock, splitFences, htmlParts, looksLikeHtml, htmlToText, wrapRawRuns, plainLayout,
...
function pagePlayHtml() {
    ...
    // 「伪沉浸」只在自由模式给（圆点与出血都只有这一档才有意义，见那一段的注释）
    const free = isFree();
    const pseudo = free && isPseudoImmersive();
    ...
    return `
        ${subnavHtml(
            ...
            `${whoPrefix(play)}第 ${path.length} 步 / 共 ${play.nodeCount || 0} 节`
                + `${pending && !here ? ' · ⏸ 有一步没写完' : ''}`,
            `${free ? pseudoImmersiveBtnHtml() : ''}${cache.index.total ? `<button class="tg-icon-btn" data-tg-act="open-dir" title="节点目录">🌳</button>` : ''}<button class="tg-icon-btn" data-tg-act="toggle-menu" title="这一局">⋯</button>`
        )}
        ${view.menuOpen ? playMenuHtml(play) : ''}
        <div class="tg-stream${pseudo ? ' is-pseudo-immersive' : ''}" data-tg-stream>
...
/* ---------- 「伪沉浸」：顶栏那个小圆点（临时件，2026-09-26）----------
   点开之后，**剧本自己画的那一节**（有沙箱框的那种）出血到屏幕边——把 .tg-stream 那左右各
   14px 的留白从它两边去掉。留给自由模式这一档：普通模式没有沙箱框，出血无从谈起，所以那个
   圆点也只在自由模式画出来。
   **默认关**（不写这个键 = 关）；**一个全局键**，跟剧本 / 这一局的记录都不发生关系。
   定位是过渡件：以后做真正的沉浸模式时，这一小节连同 CSS 里那两段 .tg-dot-btn 一起去掉，
   行为即回到「留白都在」。 */
const PSEUDO_IMMERSIVE_KEY = 'global_tg_pseudo_immersive';

function isPseudoImmersive() {
    try { return localStorage.getItem(PSEUDO_IMMERSIVE_KEY) === 'true'; } catch { return false; }
}

function togglePseudoImmersive() {
    const next = !isPseudoImmersive();
    try { localStorage.setItem(PSEUDO_IMMERSIVE_KEY, next ? 'true' : 'false'); } catch { }
    scrollTo = 'keep';
    rerender();
}

/** 顶栏那个小圆点（空心 = 关，实心 = 开）。只在自由模式的游玩页出现，排在 🌳 左边。 */
function pseudoImmersiveBtnHtml() {
    const on = isPseudoImmersive();
    return `<button class="tg-dot-btn${on ? ' is-on' : ''}" data-tg-act="toggle-pseudo-immersive"
        title="伪沉浸${on ? '：开 · 剧本画的那一节贴到屏幕边' : '：关'}" aria-pressed="${on}"></button>`;
}
 ...
 * 拼进去之前过一道 core 的 wrapRawRuns：AI 把正文写在标签里、靠裸换行分段时（很常见），
 * 那些换行会被 HTML 折成一个空格、正文挤成一坨——先给这些文字段讨回 `pre-wrap`（用户 2026-09-21 报的）。
 */
function freeHtml(aiText) {
    const parts = htmlParts(aiText);
    if (!parts.some(p => p.kind === 'html')) return '';
    const doc = parts
        .map(p => (p.kind === 'html' ? wrapRawRuns(p.text) : `<div class="tg-raw">${esc(p.text)}</div>`))
        .join('\n');
...
/**
 * 开关 ⋯ 菜单：**只加/去菜单这一个节点，正文一个节点都不碰**（同 rerenderMenu 的理由——
 * 自由模式的正文是一串 srcdoc 沙箱框，整页重画就得等它们重新加载，看上去就是正文闪一下）。
 * 看的是**翻完之后**的 `view.menuOpen`，所以开和关共用这一条。
 * 三条关闭路径都走这里：弹窗里的 ✕ 与顶栏 ⋯（同一个 act，`case 'toggle-menu'`）、
 * 点遮罩（`case 'close-menu'`）、按 ←（`handleBack` 里那一段）。⚠️ 漏掉一条，那一条就会退回整页重画。
 * 菜单在 pagePlayHtml 里排在 `.tg-stream` **前面**，所以开的时候插在它前面那个位置。
 * 事件委托挂在 .tg-page 上（根节点没换），所以不用重新 bindEvents。
 */
function syncMenu() {
    const host = currentContainer?.querySelector('.tg-modal');
    ...
    if (host) return;                              // 已经开着（兜一手，别画出两个）
    const stream = currentContainer?.querySelector('[data-tg-stream]');
    if (!stream || !cache.play) { rerender(); return; }   // 不在游玩页：退回整页重画

    stream.insertAdjacentHTML('beforebegin', playMenuHtml(cache.play));
    const body = currentContainer.querySelector('[data-tg-menu-body]');
    if (body) body.scrollTop = view.menuScroll || 0;
    view.menuAnim = false;   // 动画标记用完即废，同 afterRender 那一行
}
...
function onRootClick(event) {
    ...
    switch (el.dataset.tgAct) {
        ...
        case 'toggle-free':
            void toggleFreeMode();
            break;
        case 'toggle-pseudo-immersive':
            togglePseudoImmersive();
            break;
        case 'open-scene':
