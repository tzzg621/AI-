...
export async function bindEvents(container, state) {
  ...
  <div class="obc-shelf-cat-item ${activeCat === cat.id ? 'obc-shelf-cat-active' : ''}"
    ...
    <span class="obc-shelf-cat-name">${esc(cat.name)}</span>
    ...
    <div class="obc-shelf-book-card" data-book-id="${book.id}">
      <div class="obc-shelf-book-top">
        ...
        <div class="obc-shelf-book-info">
          <div class="obc-shelf-book-title">${esc(book.title)}</div>
          <div class="obc-shelf-book-author">${esc(book.author || '佚名')}</div>
          <div class="obc-shelf-book-tags">
            ${tags.map(t => `<span class="obc-shelf-tag">${esc(t)}</span>`).join('')}
      ...
      <div class="obc-shelf-book-synopsis">${esc(book.synopsis || '（暂无简介）')}</div>
    </div>
  `;
    }).join('');
  ...
  return new Promise((resolve) => {
    ...
    overlay.innerHTML = `
      <div style="background:white;border-radius:20px;width:280px;padding:20px 20px 30px;box-shadow:0 4px 20px rgba(0,0,0,0.15);">
        <div style="font-size:15px;font-weight:600;color:#333;margin-bottom:14px;">分类「${esc(catId)}」</div>
    ...
    <div class="obc-book-action-panel">
      <div class="obc-book-action-info">
        ...
        <div class="obc-book-action-meta">
          <div class="obc-book-action-title">${esc(book.title)}</div>
          <div class="obc-book-action-author">${esc(book.author || '佚名')} · ${(book.wordCount || 0).toLocaleString()} 字</div>
        </div>
      </div>
      ...
      <div class="obc-book-action-list">
        <div class="obc-book-action-item" data-action="category">
          ...
          <span>切换分类</span>
          <span style="margin-left:auto;color:#bbb;font-size:12px;">${esc(book.shelfCategory || '未分类')} →</span>
        </div>
  ...
  overlay.querySelector('[data-action="category"]').addEventListener('click', async () => {
    ...
    pickerOverlay.innerHTML = `
      <div style="background:white;border-radius:20px;padding:20px;width:260px;">
        <div style="font-weight:600;font-size:15px;margin-bottom:14px;">移动《${esc(book.title)}》到</div>
        ${options.map(o => `
          <label style="display:flex;align-items:center;gap:10px;padding:10px;border-radius:10px;cursor:pointer;${o === current ? 'background:#f3e5f5;' : ''}">
            <input type="radio" name="actionCatPick" value="${esc(o)}" ${o === current ? 'checked' : ''}>
            <span style="font-size:14px;">${esc(o)}</span>
          </label>
        `).join('')}
