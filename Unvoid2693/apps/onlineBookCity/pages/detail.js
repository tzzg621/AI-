...
<div class="obc-detail-header-section">
  ...
  <div class="obc-detail-info">
    <div class="obc-detail-title">${esc(book.title)}</div>
    <div class="obc-detail-author">${esc(book.author)}</div>
    <div class="obc-detail-tags">
      ${(book.tags || []).slice(0, 1).map(t => `<span class="obc-detail-tag">${esc(t)}</span>`).join('')}
      ${book.highlight ? book.highlight.split(',').map(h => `<span class="obc-detail-tag">${esc(h.trim())}</span>`).join('') : ''}
      ...
      <div class="obc-detail-section">
        ...
        <div class="obc-detail-synopsis-wrap" id="obcSynopsisWrap">
          <div class="obc-detail-synopsis ${book.synopsis ? 'obc-synopsis-clamped' : ''}"
               id="obcSynopsisText">${book.synopsis ? esc(book.synopsis).replace(/\n/g, '<br>') : '（暂无简介）'}</div>
...
${(book.chapters || []).map((ch, i) => `
  <div style="position:relative;">
    <div class="obc-detail-chapter-row" data-chapter-id="${ch.id}">
      ...
      <span class="obc-chapter-title">${esc(ch.title)}</span>
      <span class="obc-chapter-summary">${esc(ch.summary || '')}</span>
    </div>
    <span class="obc-chapter-more">⋮</span>
  </div>
`).join('')}
</div>
      </div>
  ...
  mountEl.querySelector('#obcActionUrge').addEventListener('click', async () => {
    ...
    context += (targetBook.chapters || [])
      ...
      .map((c, i) => `第${i + 1}章 ${c.title}：${c.stateSummary || c.summary || '（无摘要）'}`)
      .join('\n');
    ...
    try {
      ...
      if (ch) {
        ...
        ch.stateSummary = stateMatch ? stateMatch[1].trim() : '';   // ★ 新增
        ch.title = `第${chNum}章`;
      }
  ...
  mountEl.querySelectorAll('.obc-chapter-more').forEach(btn => {
    btn.addEventListener('click', (e) => {
      ...
      const row = btn.parentElement.querySelector('.obc-detail-chapter-row');
      ...
      menu.className = 'obc-chapter-menu';
      menu._triggerBtn = btn;
      menu.innerHTML = `
      <div class="obc-chapter-menu-item" data-action="delete">🗑️ 删除本章</div>
      <div class="obc-chapter-menu-item" data-action="regenerate">🔄 重新生成</div>
    `;

      // 定位菜单
      const rect = btn.getBoundingClientRect();
      const containerRect = mountEl.querySelector('.obc-detail-chapter-list').getBoundingClientRect();
      menu.style.position = 'fixed';
      menu.style.top = Math.min(rect.bottom + 4, window.innerHeight - 120) + 'px';
      menu.style.left = Math.max(8, Math.min(rect.right - 120, window.innerWidth - 128)) + 'px';
      ...
      menu.querySelector('[data-action="delete"]').addEventListener('click', async () => {
        menu.remove();
        activeMenu = null;
        const confirmed = await showConfirm(`确定删除「${chapter.title}」吗？`);
        if (!confirmed) return;
        const idx = book.chapters.findIndex(c => c.id === chapterId);
        if (idx !== -1) {
          book.chapters.splice(idx, 1);
          book.wordCount = Math.max(0, (book.wordCount || 0) - (chapter.content?.length || 0));
          await putBook(book);
          bindEvents(container, state);
        }
      });
      ...
      menu.querySelector('[data-action="regenerate"]').addEventListener('click', async () => {
        ...
        if (idx !== -1) {
          ...
          await putBook(book);
          // 触发催更
          const urgeBtn = mountEl.querySelector('#obcActionUrge');
          if (urgeBtn) urgeBtn.click();
        }
      });
