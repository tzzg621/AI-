...
async function editCategoryPrompt(container, state, cat) {
  ...
  overlay.innerHTML = `
    <div style="background:white;border-radius:20px;padding:24px;width:340px;max-height:80vh;display:flex;flex-direction:column;">
      <div style="font-weight:600;font-size:16px;margin-bottom:4px;">✏️ ${esc(cat.name)} · 追加自定义提示词</div>
      ...
      <div class="obc-golden-scroll" id="obcGoldenScroll">
        ...
        ${displayBooks.map(b => `
          ...
          <button class="obc-golden-remove" data-book-id="${b.id}" style="display:none;">×</button>
            ...
            <div class="obc-golden-highlight">${esc((b.tagline || b.highlight || '精彩作品').slice(0, 12))}</div>
          </div>
        `).join('')}
      </div>
    </div>
  ...
  return `
    <div class="obc-new-works-section">
      ...
      <div class="obc-new-works-grid">
        ${items.map(item => item ? `
          <div class="obc-new-work-item" data-book-id="${esc(item.id || '')}">${esc(item.title || item)}</div>
          ...
          <div class="obc-discover-card" data-book-id="${c.id || ''}">
            <div class="obc-card-top">
              ...
              <div class="obc-card-info">
                <div class="obc-card-title">${esc(c.title || '未命名')}</div>
                <div class="obc-card-author">${esc(c.author || '佚名')}</div>
              </div>
            </div>
            <div class="obc-card-summary">${esc((c.synopsis || '暂无简介').slice(0, 60))}</div>
          </div>
        `).join('')}
      </div>
    </div>
...
{
  "golden": [
    { "title": "书名", "author": "作者名", "highlight": "用逗号分隔的标签，如：'相爱相杀,强强,正剧向'（不超过五个）", "summary": "内容简介（20-300字）,模拟不同作者写简介的不同风格，需要提及主要角色介绍与关系性（如正邪cp，温和年上攻×疯批受）", "tagline": "一句话推荐语（不超过12个字），如：'白富美爱上风流浪子'"}
  ],
  "newWorks": [
    { "title": "书名", "author": "作者名", "highlight": "用逗号分隔的标签，如：'现代魔法,魔法与科学,热血战斗'（不超过五个）", "summary": "内容简介（20-200字）,需要提及主要角色介绍与关系性" }
  ],
  "cards": [
    { "title": "书名", "author": "作者名", "highlight": "用逗号分隔的标签，如：'娱乐圈,电竞,同人二创'（不超过五个）", "summary": "内容简介（20-200字）,需要提及主要角色介绍与关系性" }
  ]
}
...
- 所有书名、作者、看点、简介需贴合"${name}"类别`;
  ...
  <div style="text-align:center;padding:60px 20px;">
    <div style="font-size:40px;margin-bottom:12px;">❌</div>
    <div style="color:#e53935;font-size:14px;margin-bottom:16px;">生成失败：${esc(e.message)}</div>
