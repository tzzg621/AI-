...
class RelationshipApp {
    ...
    loop(timestamp) {
        ...
        const interactionRunning =
            Boolean(this.interaction?.dragging)
            || Boolean(this.interaction?.isPanning);

        // 布局、中心迁移或用户操作期间才继续逐帧运行。
        if (
            layoutRunning
            || isCenterArriving
            || interactionRunning
